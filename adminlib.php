...
/**
 * This file contains the classes for the admin settings of the review module.
 *
 * @package   mod_review
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_admin_page_manage_review_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new moodle_url('/mod/review/adminmanageplugins.php', array('subtype'=>$subtype));
        parent::__construct('manage' . $subtype . 'plugins',
                            get_string('manage' . $subtype . 'plugins', 'review'),
                            $url);
    }
...
class review_plugin_manager {
    ...
    /**
     * Constructor for this review plugin manager
     * @param string $subtype - either reviewsubmission or reviewfeedback
     */
    public function __construct($subtype) {
        $this->pageurl = new moodle_url('/mod/review/adminmanageplugins.php', array('subtype'=>$subtype));
        $this->subtype = $subtype;
    }
    ...
    private function view_plugins_table() {
        ...
        $table->define_columns(array('pluginname', 'version', 'hideshow', 'order',
                'settings', 'uninstall'));
        $table->define_headers(array(get_string($this->subtype . 'pluginname', 'review'),
                get_string('version'), get_string('hideshow', 'review'),
        ...
        $shortsubtype = substr($this->subtype, strlen('review'));
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            $exists = file_exists($CFG->dirroot . '/mod/review/' . $shortsubtype . '/' . $plugin . '/settings.php');
            if ($row[1] != '' && $exists) {
    ...
    private function view_header() {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('manage' . $this->subtype . 'plugins', 'review'));
    }
