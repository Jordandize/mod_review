 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_feedback_editpdf extends review_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'reviewfeedback_editpdf');
    }
     ...
     * @param bool $readonly
     * @return reviewfeedback_editpdf_widget
     */
    public function get_widget($userid, $grade, $readonly) {
        ...
        $feedbackfile = document_services::get_feedback_document($this->review->get_instance()->id,
                                                                 $userid,
                                                                 $attempt);
        ...
        if ($files = $fs->get_area_files($syscontext->id,
            ...
            foreach ($files as $file) {
                ...
                if ($filename !== '.') {
                    ...
                    $existingfile = $fs->get_file($this->review->get_context()->id,
                                                  'reviewfeedback_editpdf',
                                                  'stamps',
                                                  $grade->id,
                                                  '/',
                    ...
                    if (!$existingfile) {
                        ...
                        $newrecord->contextid = $this->review->get_context()->id;
        ...
        if ($files = $fs->get_area_files($this->review->get_context()->id,
                                         'reviewfeedback_editpdf',
                                         'stamps',
                                         $grade->id,
                                         "filename",
            ...
            foreach ($files as $file) {
                $filename = $file->get_filename();
                if ($filename !== '.') {
                    $url = moodle_url::make_pluginfile_url($this->review->get_context()->id,
                                                   'reviewfeedback_editpdf',
                                                   'stamps',
                                                   $grade->id,
                                                   '/',
        ...
        $widget = new reviewfeedback_editpdf_widget($this->review->get_instance()->id,
                                                    $userid,
                                                    $attempt,
                                                    $url,
                                                    $filename,
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        $renderer = $PAGE->get_renderer('reviewfeedback_editpdf');

        $widget = $this->get_widget($userid, $grade, false);

        $html = $renderer->render($widget);
        $mform->addElement('static', 'editpdf', get_string('editpdf', 'reviewfeedback_editpdf'), $html);
        $mform->addHelpButton('editpdf', 'editpdf', 'reviewfeedback_editpdf');
    ...
    public function is_feedback_modified(stdClass $grade, stdClass $data) {
        ...
        if (!empty($data->editpdf_source_userid)) {
            ...
            $sourcegrade = $this->review->get_user_grade($sourceuserid, true, $grade->attemptnumber);
            $pagenumbercount = document_services::page_number_for_attempt($this->review, $sourceuserid, $sourcegrade->attemptnumber);
    ...
    public function save(stdClass $grade, stdClass $data) {
        ...
        if (!empty($data->editpdf_source_userid)) {
            ...
            if ($sourceuserid != $grade->userid) {
                page_editor::copy_drafts_from_to($this->review, $grade, $sourceuserid);
            }
        }
        if (page_editor::has_annotations_or_comments($grade->id, true)) {
            document_services::generate_feedback_document($this->review, $grade->userid, $grade->attemptnumber);
        }
    ...
    public function view(stdClass $grade) {
        ...
        if (page_editor::has_annotations_or_comments($grade->id, false)) {
            $html = $this->review->render_area_files('reviewfeedback_editpdf',
                                                         document_services::FINAL_PDF_FILEAREA,
                                                         $grade->id);
    ...
    public function is_empty(stdClass $grade) {
        ...
        $comments = $DB->count_records('reviewfeedback_editpdf_cmnt', array('gradeid'=>$grade->id, 'draft'=>0));
        $annotations = $DB->count_records('reviewfeedback_editpdf_annot', array('gradeid'=>$grade->id, 'draft'=>0));
    ...
    public function delete_instance() {
        global $DB;
        $grades = $DB->get_records('review_grades', array('review'=>$this->review->get_instance()->id), '', 'id');
        if ($grades) {
            list($gradeids, $params) = $DB->get_in_or_equal(array_keys($grades), SQL_PARAMS_NAMED);
            $DB->delete_records_select('reviewfeedback_editpdf_annot', 'gradeid ' . $gradeids, $params);
            $DB->delete_records_select('reviewfeedback_editpdf_cmnt', 'gradeid ' . $gradeids, $params);
        }
        return true;
    }
    ...
    public function is_available() {
        if ($this->enabledcache === null) {
            $testpath = reviewfeedback_editpdf\pdf::test_gs_path(false);
            $this->enabledcache = ($testpath->status == reviewfeedback_editpdf\pdf::GSPATH_OK);
        }
