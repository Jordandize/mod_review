...
/**
 * This file contains the editor class for the reviewfeedback_editpdf plugin
 *
...
class page_editor {
    ...
    public static function get_comments($gradeid, $pageno, $draft) {
        ...
        $records = $DB->get_records('reviewfeedback_editpdf_cmnt', $params, 'y, x');
    ...
    public static function set_comments($gradeid, $pageno, $comments) {
        ...
        $DB->delete_records('reviewfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'pageno'=>$pageno, 'draft'=>1));
    ...
    public static function get_comment($commentid) {
        $record = $DB->get_record('reviewfeedback_editpdf_cmnt', array('id'=>$commentid), '*', IGNORE_MISSING);
        if ($record) {
    ...
    public static function add_comment(comment $comment) {
        global $DB;
        $comment->id = null;
        return $DB->insert_record('reviewfeedback_editpdf_cmnt', $comment);
    }
    ...
    public static function remove_comment($commentid) {
        global $DB;
        return $DB->delete_records('reviewfeedback_editpdf_cmnt', array('id'=>$commentid));
    }
    ...
    public static function get_annotations($gradeid, $pageno, $draft) {
        ...
        $annotations = array();
        $records = $DB->get_records('reviewfeedback_editpdf_annot', $params);
        foreach ($records as $record) {
    ...
    public static function set_annotations($gradeid, $pageno, $annotations) {
        ...
        $DB->delete_records('reviewfeedback_editpdf_annot', array('gradeid' => $gradeid, 'pageno' => $pageno, 'draft' => 1));
        $added = 0;
    ...
    public static function get_annotation($annotationid) {
        ...
        $record = $DB->get_record('reviewfeedback_editpdf_annot', array('id'=>$annotationid), '*', IGNORE_MISSING);
        if ($record) {
    ...
    public static function unrelease_drafts($gradeid) {
        ...
        $result = $DB->delete_records('reviewfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>0));
        $result = $DB->delete_records('reviewfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>0)) && $result;
        return $result;
    }
    ...
    public static function release_drafts($gradeid) {
        ...
        $DB->delete_records('reviewfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>0));
        $DB->delete_records('reviewfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>0));

        // Copy all the draft annotations and comments to non-drafts.
        $records = $DB->get_records('reviewfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>1));
        foreach ($records as $record) {
            unset($record->id);
            $record->draft = 0;
            $DB->insert_record('reviewfeedback_editpdf_annot', $record);
        }
        $records = $DB->get_records('reviewfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>1));
        foreach ($records as $record) {
            unset($record->id);
            $record->draft = 0;
            $DB->insert_record('reviewfeedback_editpdf_cmnt', $record);
        }
    ...
    public static function has_annotations_or_comments($gradeid, $includedraft) {
        ...
        if ($DB->count_records('reviewfeedback_editpdf_cmnt', $params)) {
            return true;
        }
        if ($DB->count_records('reviewfeedback_editpdf_annot', $params)) {
            return true;
        }
        return false;
    }
    ...
    public static function revert_drafts($gradeid) {
        ...
        // Delete the previous non-draft annotations and comments.
        $DB->delete_records('reviewfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>1));
        $DB->delete_records('reviewfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>1));

        // Copy all the draft annotations and comments to non-drafts.
        $records = $DB->get_records('reviewfeedback_editpdf_annot', array('gradeid'=>$gradeid, 'draft'=>0));
        foreach ($records as $record) {
            unset($record->id);
            $record->draft = 0;
            $DB->insert_record('reviewfeedback_editpdf_annot', $record);
        }
        $records = $DB->get_records('reviewfeedback_editpdf_cmnt', array('gradeid'=>$gradeid, 'draft'=>0));
        foreach ($records as $record) {
            unset($record->id);
            $record->draft = 0;
            $DB->insert_record('reviewfeedback_editpdf_annot', $record);
    ...
    public static function add_annotation(annotation $annotation) {
        ...
        $annotation->id = null;
        return $DB->insert_record('reviewfeedback_editpdf_annot', $annotation);
    }
    ...
    public static function remove_annotation($annotationid) {
        ...
        return $DB->delete_records('reviewfeedback_editpdf_annot', array('id'=>$annotationid));
    }
     ...
     * @return bool
     */
    public static function copy_drafts_from_to($review, $grade, $sourceuserid) {
        global $DB;

        // Delete any existing annotations and comments from current user.
        $DB->delete_records('reviewfeedback_editpdf_annot', array('gradeid' => $grade->id));
        $DB->delete_records('reviewfeedback_editpdf_cmnt', array('gradeid' => $grade->id));
        // Get gradeid, annotations and comments from sourceuserid.
        $sourceusergrade = $review->get_user_grade($sourceuserid, true, $grade->attemptnumber);
        $annotations = $DB->get_records('reviewfeedback_editpdf_annot', array('gradeid' => $sourceusergrade->id, 'draft' => 1));
        $comments = $DB->get_records('reviewfeedback_editpdf_cmnt', array('gradeid' => $sourceusergrade->id, 'draft' => 1));
        ...
        foreach ($annotations as $annotation) {
            ...
            $DB->insert_record('reviewfeedback_editpdf_annot', $annotation);
        }
        foreach ($comments as $comment) {
            $comment->gradeid = $grade->id;
            $DB->insert_record('reviewfeedback_editpdf_cmnt', $comment);
        }
     ...
     * @param int $contextid The ID of the context for the review.
    ...
    public static function delete_draft_content($gradeid) {
        ...
        $result = $DB->delete_records('reviewfeedback_editpdf_annot', $conditions);
        $result = $result && $DB->delete_records('reviewfeedback_editpdf_cmnt', $conditions);
        return $result;
    }
    ...
    public static function set_page_rotation($gradeid, $pageno, $isrotated, $pathnamehash, $degree = 0) {
        ...
        if ($oldrecord == null) {
            ...
            $DB->insert_record('reviewfeedback_editpdf_rot', $record, false);
        } else {
            $oldrecord->isrotated = $isrotated;
            $oldrecord->pathnamehash = $pathnamehash;
            $oldrecord->degree = $degree;
            $DB->update_record('reviewfeedback_editpdf_rot', $oldrecord, false);
        }
    }
    ...
    public static function get_page_rotation($gradeid, $pageno) {
        global $DB;
        $result = $DB->get_record('reviewfeedback_editpdf_rot', array('gradeid' => $gradeid, 'pageno' => $pageno));
        return $result;
    }
