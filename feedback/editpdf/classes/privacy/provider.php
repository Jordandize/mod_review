...
namespace reviewfeedback_editpdf\privacy;
...
use \mod_review\privacy\reviewfeedback_provider;
...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table('reviewfeedback_editpdf_quick', $quickdata, 'privacy:metadata:tablepurpose');
    ...
    public static function export_feedback_user_data(review_plugin_request_data $exportdata) {
        ...
        $currentpath[] = get_string('privacy:path', 'reviewfeedback_editpdf');
        $review = $exportdata->get_review();
        $plugin = $review->get_plugin_by_type('reviewfeedback', 'editpdf');
        $fileareas = $plugin->get_file_areas();
        $grade = $exportdata->get_pluginobject();
        foreach ($fileareas as $filearea => $notused) {
            writer::with_context($exportdata->get_context())
                    ->export_area_files($currentpath, 'reviewfeedback_editpdf', $filearea, $grade->id);
        }
    }
    ...
    public static function delete_feedback_for_context(review_plugin_request_data $requestdata) {
        ...
        foreach ($fileareas as $filearea => $notused) {
            // Delete pdf files.
            $fs->delete_area_files($requestdata->get_context()->id, 'reviewfeedback_editpdf', $filearea);
        }
    ...
    public static function delete_feedback_for_grades(review_plugin_request_data $deletedata) {
        ...
        foreach ($fileareas as $filearea => $notused) {
            // Delete pdf files.
            $fs->delete_area_files_select($deletedata->get_context()->id, 'reviewfeedback_editpdf', $filearea, $sql, $params);
        }

        // Remove table entries.
        $DB->delete_records_select('reviewfeedback_editpdf_annot', "gradeid $sql", $params);
        $DB->delete_records_select('reviewfeedback_editpdf_cmnt', "gradeid $sql", $params);
        $DB->delete_records_select('reviewfeedback_editpdf_rot', "gradeid $sql", $params);
        // Submission records in reviewfeedback_editpdf_queue will be cleaned up in a scheduled task
    }
}
