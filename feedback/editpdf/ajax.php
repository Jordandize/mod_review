...
require_login($review->get_course(), false, $cm);
...
if ($action === 'pollconversions') {
    ...
    if (!has_capability('mod/review:grade', $context)) {
        ...
        $readonly = true;
        $draft = false;
        require_capability('mod/review:submit', $context);
    }
    ...
    $combineddocument = document_services::get_combined_document_for_attempt($review, $userid, $attemptnumber);
    ...
    if (in_array($response->status, $readystatuslist)) {
        ...
        $pages = document_services::get_page_images_for_attempt($review,
                                                                $userid,
                                                                $attemptnumber,
                                                                $readonly);
    ...
    if ($added != count($page->comments)) {
        array_push($response->errors, get_string('couldnotsavepage', 'reviewfeedback_editpdf', $index+1));
    }
    ...
    $response = new stdClass();
    $grade = $review->get_user_grade($userid, true, $attemptnumber);
    $file = document_services::generate_feedback_document($review, $userid, $attemptnumber);

    $response->url = '';
    if ($file) {
        $url = moodle_url::make_pluginfile_url($review->get_context()->id,
                                               'reviewfeedback_editpdf',
    ...
    $grade = $review->get_user_grade($userid, true, $attemptnumber);
    $result = document_services::delete_feedback_document($review, $userid, $attemptnumber);
    ...
    $filearea = document_services::PAGE_IMAGE_FILEAREA;
    $pagefile = document_services::rotate_page($review, $userid, $attemptnumber, $index, $rotateleft);
    $page = new stdClass();
