 ...
 * @return bool
 */
function xmldb_reviewfeedback_editpdf_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2017022700) {
        ...
        $where = "component='reviewfeedback_editpdf' AND filearea = 'importhtml' AND " . $sqllike;
        ...
        upgrade_plugin_savepoint(true, 2017022700, 'reviewfeedback', 'editpdf');
    }
    ...
    if ($oldversion < 2018051401) {
        $table = new xmldb_table('reviewfeedback_editpdf_queue');
    ...
    if ($oldversion < 2019010800) {
        // Define table reviewfeedback_editpdf_rot to be created.
        $table = new xmldb_table('reviewfeedback_editpdf_rot');

        // Adding fields to table reviewfeedback_editpdf_rot.
        ...
        // Adding keys to table reviewfeedback_editpdf_rot.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('gradeid', XMLDB_KEY_FOREIGN, ['gradeid'], 'review_grades', ['id']);

        // Adding indexes to table reviewfeedback_editpdf_rot.
        $table->add_index('gradeid_pageno', XMLDB_INDEX_UNIQUE, ['gradeid', 'pageno']);

        // Conditionally launch create table for reviewfeedback_editpdf_rot.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Editpdf savepoint reached.
        upgrade_plugin_savepoint(true, 2019010800, 'reviewfeedback', 'editpdf');
    }
