...
// File areas for file feedback review.
define('REVIEWFEEDBACK_FILE_FILEAREA', 'feedback_files');
define('REVIEWFEEDBACK_FILE_BATCH_FILEAREA', 'feedback_files_batch');
define('REVIEWFEEDBACK_FILE_IMPORT_FILEAREA', 'feedback_files_import');
define('REVIEWFEEDBACK_FILE_MAXSUMMARYFILES', 5);
define('REVIEWFEEDBACK_FILE_MAXSUMMARYUSERS', 5);
define('REVIEWFEEDBACK_FILE_MAXFILEUNZIPTIME', 120);
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_feedback_file extends review_feedback_plugin {
    ...
    public function get_name() {
        return get_string('file', 'reviewfeedback_file');
    }
    ...
    public function get_file_feedback($gradeid) {
        global $DB;
        return $DB->get_record('reviewfeedback_file', array('grade'=>$gradeid));
    }
    ...
    public function is_feedback_modified(stdClass $grade, stdClass $data) {
        ...
        if (isset($filekey)) {
            ...
            $filecount = $this->count_files($grade->id, REVIEWFEEDBACK_FILE_FILEAREA);
    ...
    public function update_file_count($grade) {
        ...
        if ($filefeedback) {
            $filefeedback->numfiles = $this->count_files($grade->id, REVIEWFEEDBACK_FILE_FILEAREA);
            return $DB->update_record('reviewfeedback_file', $filefeedback);
        } else {
            $filefeedback = new stdClass();
            $filefeedback->numfiles = $this->count_files($grade->id, REVIEWFEEDBACK_FILE_FILEAREA);
            $filefeedback->grade = $grade->id;
            $filefeedback->review = $this->review->get_instance()->id;
            return $DB->insert_record('reviewfeedback_file', $filefeedback) > 0;
        }
    }
    ...
    public function view_summary(stdClass $grade, & $showviewlink) {
        ...
        $count = $this->count_files($grade->id, REVIEWFEEDBACK_FILE_FILEAREA);

        // Show a view all link if the number of files is over this limit.
        $showviewlink = $count > REVIEWFEEDBACK_FILE_MAXSUMMARYFILES;

        if ($count <= REVIEWFEEDBACK_FILE_MAXSUMMARYFILES) {
            return $this->review->render_area_files('reviewfeedback_file',
                                                        REVIEWFEEDBACK_FILE_FILEAREA,
                                                        $grade->id);
        } else {
            return get_string('countfiles', 'reviewfeedback_file', $count);
        }
    }
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('reviewfeedback_file',
    ...
    public function is_empty(stdClass $grade) {
        return $this->count_files($grade->id, REVIEWFEEDBACK_FILE_FILEAREA) == 0;
    }
    ...
    public function get_file_areas() {
        return array(REVIEWFEEDBACK_FILE_FILEAREA=>$this->get_name());
    }
    ...
    /**
     * Upgrade the feedback from the old review to the new one.
     *
    ...
    public function upgrade(context $oldcontext,
        ...
        $filefeedback->grade = $grade->id;
        $filefeedback->review = $this->review->get_instance()->id;
        if (!$DB->insert_record('reviewfeedback_file', $filefeedback) > 0) {
            $log .= get_string('couldnotconvertgrade', 'mod_review', $grade->userid);
            return false;
        }
        return true;
    }
    ...
    public function get_grading_batch_operations() {
        return array('uploadfiles'=>get_string('uploadfiles', 'reviewfeedback_file'));
    }
    ...
    public function view_batch_upload_files($users) {
        ...
        require_once($CFG->dirroot . '/mod/review/feedback/file/batchuploadfilesform.php');
        require_once($CFG->dirroot . '/mod/review/renderable.php');

        $formparams = array('cm'=>$this->review->get_course_module()->id,
                            'users'=>$users,
        ...
        foreach ($users as $userid) {
            if ($usercount >= REVIEWFEEDBACK_FILE_MAXSUMMARYUSERS) {
                $moreuserscount = count($users) - REVIEWFEEDBACK_FILE_MAXSUMMARYUSERS;
                $usershtml .= get_string('moreusers', 'reviewfeedback_file', $moreuserscount);
                break;
            }
            $user = $DB->get_record('user', array('id'=>$userid), '*', MUST_EXIST);

            $usersummary = new review_user_summary($user,
                                                   $this->review->get_course()->id,
                                                   has_capability('moodle/site:viewfullnames',
                                                   $this->review->get_course_context()),
                                                   $this->review->is_blind_marking(),
                                                   $this->review->get_uniqueid_for_user($user->id),
                                                   get_extra_user_fields($this->review->get_context()));
            $usershtml .= $this->review->get_renderer()->render($usersummary);
            $usercount += 1;
        }

        $formparams['usershtml'] = $usershtml;

        $mform = new reviewfeedback_file_batch_upload_files_form(null, $formparams);
        ...
        if ($mform->is_cancelled()) {
            ...
            $data = file_postupdate_standard_filemanager($data,
                                                         ...
                                                         $this->review->get_context(),
                                                         'reviewfeedback_file',
                                                         REVIEWFEEDBACK_FILE_BATCH_FILEAREA,
                                                         $USER->id);
            $fs = get_file_storage();

            // Now copy each of these files to the users feedback file area.
            foreach ($users as $userid) {
                $grade = $this->review->get_user_grade($userid, true);
                ...
                if ($filefeedback) {
                    $filefeedback->numfiles = $this->count_files($grade->id,
                                                                 REVIEWFEEDBACK_FILE_FILEAREA);
                    $DB->update_record('reviewfeedback_file', $filefeedback);
                } else {
                    $filefeedback = new stdClass();
                    $filefeedback->numfiles = $this->count_files($grade->id,
                                                                 REVIEWFEEDBACK_FILE_FILEAREA);
                    $filefeedback->grade = $grade->id;
                    $filefeedback->review = $this->review->get_instance()->id;
                    $DB->insert_record('reviewfeedback_file', $filefeedback);
                }
            }
            ...
            $header = new review_header($this->review->get_instance(),
                                        $this->review->get_context(),
                                        false,
                                        $this->review->get_course_module()->id,
                                        get_string('batchuploadfiles', 'reviewfeedback_file'));
            $o = '';
            $o .= $this->review->get_renderer()->render($header);
            $o .= $this->review->get_renderer()->render(new review_form('batchuploadfiles', $mform));
            $o .= $this->review->get_renderer()->render_footer();
        }
    ...
    public function view_upload_zip() {
        ...
        require_capability('mod/review:grade', $this->review->get_context());
        require_once($CFG->dirroot . '/mod/review/feedback/file/uploadzipform.php');
        require_once($CFG->dirroot . '/mod/review/feedback/file/importziplib.php');
        require_once($CFG->dirroot . '/mod/review/feedback/file/importzipform.php');

        $formparams = array('context'=>$this->review->get_context(),
                            'cm'=>$this->review->get_course_module()->id);
        $mform = new reviewfeedback_file_upload_zip_form(null, $formparams);
        ...
        $importer = new reviewfeedback_file_zip_importer();
        ...
        if ($mform->is_cancelled()) {
            ...
            $params = array('review'=>$this->review, 'importer'=>$importer);

            $mform = new reviewfeedback_file_import_zip_form(null, $params);
            if ($mform->is_cancelled()) {
                $importer->delete_import_files($contextid);
                $urlparams = array('id'=>$this->review->get_course_module()->id,
                                   'action'=>'grading');
            ...
            $o .= $importer->import_zip_files($this->review, $this);
            $importer->delete_import_files($contextid);
        } else if (($data = $mform->get_data()) &&
                   ($zipfile = $mform->save_stored_file('feedbackzip',
                                                        $contextid,
                                                        'reviewfeedback_file',
                                                        REVIEWFEEDBACK_FILE_IMPORT_FILEAREA,
                                                        $USER->id,
                                                        '/',
                                                        'import.zip',
                                                        true))) {
            ...
            $header = new review_header($this->review->get_instance(),
                                        ...
                                        false,
                                        $this->review->get_course_module()->id,
                                        get_string('confirmuploadzip', 'reviewfeedback_file'));
            $o .= $renderer->render($header);
            $o .= $renderer->render(new review_form('confirmimportzip', $mform));
            ...
            $header = new review_header($this->review->get_instance(),
                                        ...
                                        false,
                                        $this->review->get_course_module()->id,
                                        get_string('uploadzip', 'reviewfeedback_file'));
            $o .= $renderer->render($header);
            $o .= $renderer->render(new review_form('uploadfeedbackzip', $mform));
            $o .= $renderer->render_footer();
        }
    ...
    /**
     * Called by the review module when someone chooses something from the
    ...
    public function get_grading_actions() {
        return array('uploadzip'=>get_string('uploadzip', 'reviewfeedback_file'));
    }
