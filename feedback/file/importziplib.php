 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class reviewfeedback_file_zip_importer {

    /**
     * Is this filename valid (contains a unique participant ID) for import?
     *
     * @param review $review - The review instance
     ...
     * @param review_plugin $plugin - Set to the plugin that exported the file
     ...
     */
    public function is_valid_filename_for_import($review, $fileinfo, $participants, & $user, & $plugin, & $filename) {
        ...
        $filename = $info[4];
        $plugin = $review->get_plugin_by_type($info[2], $info[3]);
     ...
     * @param review_plugin $plugin The matching plugin from the filename
     ...
     */
    public function is_file_modified($review, $user, $plugin, $filename, $fileinfo) {
        $sg = null;

        if ($plugin->get_subtype() == 'reviewsubmission') {
            $sg = $review->get_user_submission($user->id, false);
        } else if ($plugin->get_subtype() == 'reviewfeedback') {
            $sg = $review->get_user_grade($user->id, false);
        } else {
            return false;
        }
     ...
     *
     * @param int $contextid - The context id of this review instance
     ...
     * @param int $contextid The context for this review
    ...
    public function extract_files_from_zip($zipfile, $contextid) {
        ...
        core_php_time_limit::raise(REVIEWFEEDBACK_FILE_MAXFILEUNZIPTIME);
     ...
     *
     * @param review $review - The review instance
     * @param review_feedback_file $fileplugin - The file feedback plugin
     * @return string - The html response
     */
    public function import_zip_files($review, $fileplugin) {
        global $CFG, $PAGE, $DB;
        ...
        foreach ($files as $unzippedfile) {
            ...
            if ($this->is_valid_filename_for_import($review, $unzippedfile, $participants, $user, $plugin, $filename)) {
                if ($this->is_file_modified($review, $user, $plugin, $filename, $unzippedfile)) {
                    $grade = $review->get_user_grade($user->id, true);
                    ...
                    if ($oldfile = $fs->get_file($contextid,
                        ...
                        $newfilerecord->component = 'reviewfeedback_file';
                        $newfilerecord->filearea = REVIEWFEEDBACK_FILE_FILEAREA;
                    ...
                    $review->notify_grade_modified($grade);
                }
            }
        }

        require_once($CFG->dirroot . '/mod/review/feedback/file/renderable.php');
        $importsummary = new reviewfeedback_file_import_summary($review->get_course_module()->id,
                                                            count($userswithnewfeedback),
                                                            $feedbackfilesadded,
                                                            $feedbackfilesupdated);

        $reviewrenderer = $review->get_renderer();
        $renderer = $PAGE->get_renderer('reviewfeedback_file');

        $o = '';

        $o .= $reviewrenderer->render(new review_header($review->get_instance(),
                                                        $review->get_context(),
                                                        false,
                                                        $review->get_course_module()->id,
                                                        get_string('uploadzipsummary', 'reviewfeedback_file')));

        $o .= $renderer->render($importsummary);

        $o .= $reviewrenderer->render_footer();
        return $o;
    }
