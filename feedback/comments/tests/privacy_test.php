...
/**
 * Unit tests for mod/review/feedback/comments/classes/privacy/
 *
 * @copyright  2018 Adrian Greeve <adrian@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class reviewfeedback_comments_privacy_testcase extends \mod_review\tests\mod_review_privacy_testcase {
    ...
    protected function create_feedback($review, $student, $teacher, $submissiontext, $feedbacktext) {
        ...
        file_prepare_draft_area($draftitemid, $context->id, REVIEWFEEDBACK_COMMENTS_COMPONENT,
            REVIEWFEEDBACK_COMMENTS_FILEAREA, $grade->id);
        ...
        $feedbackdata->reviewfeedbackcomments_editor = [
            'text' => $feedbacktext,
            'format' => FORMAT_HTML,
            'itemid' => $draftitemid
        ];
    ...
    public function test_get_metadata() {
        $collection = new \core_privacy\local\metadata\collection('reviewfeedback_comments');
        $collection = \reviewfeedback_comments\privacy\provider::get_metadata($collection);
    ...
    public function test_export_feedback_user_data() {
        ...
        \reviewfeedback_comments\privacy\provider::export_feedback_user_data($exportdata);
        ...
        $exportdata = new \mod_review\privacy\review_plugin_request_data($context, $review, $grade, [], $user2);
        \reviewfeedback_comments\privacy\provider::export_feedback_user_data($exportdata);
        $this->assertContains($feedbacktext, $writer->get_data(['Feedback comments'])->commenttext);
    ...
    public function test_delete_feedback_for_context() {
        ...
        $fs = new file_storage();
        $files = $fs->get_area_files($review->get_context()->id, REVIEWFEEDBACK_COMMENTS_COMPONENT,
        ...
        reviewfeedback_comments\privacy\provider::delete_feedback_for_context($requestdata);
    ...
    public function test_delete_feedback_for_grade() {
        ...
        $requestdata = new \mod_review\privacy\review_plugin_request_data($context, $review, $grade1, [], $user1);
        reviewfeedback_comments\privacy\provider::delete_feedback_for_grade($requestdata);
    ...
    public function test_delete_feedback_for_grades() {
        ...
        $feedbacktext = '<p>Comment for third student in the second review.</p>';
        list($plugin4, $grade4) = $this->create_feedback($review2, $user3, $user5, 'Submission text', $feedbacktext);
        $feedbacktext = '<p>Comment for fourth student in the second review.</p>';
        ...
        $fs = new file_storage();
        // 6 including directories for review 1.
        // 4 including directories for review 2.
        $this->assertCount(6, $fs->get_area_files($review1->get_context()->id,
                REVIEWFEEDBACK_COMMENTS_COMPONENT, REVIEWFEEDBACK_COMMENTS_FILEAREA));
        $this->assertCount(4, $fs->get_area_files($review2->get_context()->id,
                REVIEWFEEDBACK_COMMENTS_COMPONENT, REVIEWFEEDBACK_COMMENTS_FILEAREA));

        $deletedata = new \mod_review\privacy\review_plugin_request_data($review1->get_context(), $review1);
        $deletedata->set_userids([$user1->id, $user3->id]);
        $deletedata->populate_submissions_and_grades();
        reviewfeedback_comments\privacy\provider::delete_feedback_for_grades($deletedata);
        ...
        // We have deleted two from review 1, and none from review 2.
        // 2 including directories for review 1.
        // 4 including directories for review 2.
        $this->assertCount(2, $fs->get_area_files($review1->get_context()->id,
                REVIEWFEEDBACK_COMMENTS_COMPONENT, REVIEWFEEDBACK_COMMENTS_FILEAREA));
        $this->assertCount(4, $fs->get_area_files($review2->get_context()->id,
                REVIEWFEEDBACK_COMMENTS_COMPONENT, REVIEWFEEDBACK_COMMENTS_FILEAREA));
    }
