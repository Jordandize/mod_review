...
define('REVIEWFEEDBACK_COMMENTS_COMPONENT', 'reviewfeedback_comments');

// File area for feedback comments.
define('REVIEWFEEDBACK_COMMENTS_FILEAREA', 'feedback');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_feedback_comments extends review_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'reviewfeedback_comments');
    }
    ...
    public function get_feedback_comments($gradeid) {
        global $DB;
        return $DB->get_record('reviewfeedback_comments', array('grade'=>$gradeid));
    }
    ...
    public function get_quickgrading_html($userid, $grade) {
        ...
        $pluginname = get_string('pluginname', 'reviewfeedback_comments');
    ...
    public function is_feedback_modified(stdClass $grade, stdClass $data) {
        ...
        $formtext = $data->reviewfeedbackcomments_editor['text'];

        // Need to convert the form text to use @@PLUGINFILE@@ and format it so we can compare it with what is stored in the DB.
        if (isset($data->reviewfeedbackcomments_editor['itemid'])) {
            $formtext = file_rewrite_urls_to_pluginfile($formtext, $data->reviewfeedbackcomments_editor['itemid']);
    ...
    public function get_editor_fields() {
        return array('comments' => get_string('pluginname', 'reviewfeedback_comments'));
    }
    ...
    public function set_editor_text($name, $value, $gradeid) {
        ...
        if ($name == 'comments') {
            ...
            if ($feedbackcomment) {
                ...
                return $DB->update_record('reviewfeedback_comments', $feedbackcomment);
            } else {
                $feedbackcomment = new stdClass();
                $feedbackcomment->commenttext = $value;
                $feedbackcomment->commentformat = FORMAT_HTML;
                $feedbackcomment->grade = $gradeid;
                $feedbackcomment->review = $this->review->get_instance()->id;
                return $DB->insert_record('reviewfeedback_comments', $feedbackcomment) > 0;
            }
        }
    ...
    public function save_settings(stdClass $data) {
        $this->set_config('commentinline', !empty($data->reviewfeedback_comments_commentinline));
        return true;
    }
    ...
    public function get_settings(MoodleQuickForm $mform) {
        ...
        if ($default === false) {
            ...
            $default = get_config('reviewfeedback_comments', 'inline');
        }
        $mform->addElement('selectyesno',
                           'reviewfeedback_comments_commentinline',
                           get_string('commentinline', 'reviewfeedback_comments'));
        $mform->addHelpButton('reviewfeedback_comments_commentinline', 'commentinline', 'reviewfeedback_comments');
        $mform->setDefault('reviewfeedback_comments_commentinline', $default);
        // Disable comment online if comment feedback plugin is disabled.
        $mform->hideIf('reviewfeedback_comments_commentinline', 'reviewfeedback_comments_enabled', 'notchecked');
   }
    ...
    protected function convert_submission_text_to_feedback($submission, $data, $grade) {
        ...
        foreach ($this->review->get_submission_plugins() as $plugin) {
            ...
            if ($plugin->is_enabled() && $plugin->is_visible() && !$plugin->is_empty($submission) && !empty($fields)) {
                ...
                if ($files = $plugin->get_files($submission, $user)) {
                    $fs = get_file_storage();
                    $component = 'reviewfeedback_comments';
                    $filearea = REVIEWFEEDBACK_COMMENTS_FILEAREA;
                    $itemid = $grade->id;
                    $fieldupdates = [
                        'component' => $component,
                        'filearea' => $filearea,
        ...
        $data->reviewfeedbackcomments = $text;
        $data->reviewfeedbackcommentsformat = $format;
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        $submission = $this->review->get_user_submission($userid, false);
        ...
        if ($feedbackcomments && !empty($feedbackcomments->commenttext)) {
            $data->reviewfeedbackcomments = $feedbackcomments->commenttext;
            $data->reviewfeedbackcommentsformat = $feedbackcomments->commentformat;
        } else {
            ...
            if (!empty($commentinlinenabled) && $submission) {
                ...
                $data->reviewfeedbackcomments = '';
                $data->reviewfeedbackcommentsformat = FORMAT_HTML;
            }
        }

        file_prepare_standard_editor(
            $data,
            'reviewfeedbackcomments',
        ...
        $mform->addElement('editor', 'reviewfeedbackcomments_editor', $this->get_name(), null, $this->get_editor_options());
    ...
    public function save(stdClass $grade, stdClass $data) {
        ...
        $feedbackcomment = $this->get_feedback_comments($grade->id);
        if ($feedbackcomment) {
            $feedbackcomment->commenttext = $data->reviewfeedbackcomments;
            $feedbackcomment->commentformat = $data->reviewfeedbackcommentsformat;
            return $DB->update_record('reviewfeedback_comments', $feedbackcomment);
        } else {
            $feedbackcomment = new stdClass();
            $feedbackcomment->commenttext = $data->reviewfeedbackcomments;
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldreview, & $log) {
        if ($oldreview->reviewtype == 'online') {
            $this->set_config('commentinline', $oldreview->var1);
            return true;
        }
        return true;
    }
    ...
    public function upgrade(context $oldcontext,
        ...
        $feedbackcomments->review = $this->review->get_instance()->id;
        if (!$DB->insert_record('reviewfeedback_comments', $feedbackcomments) > 0) {
     ...
     * of the comment
     *
     * Only one feedback plugin can push comments to the gradebook and that is chosen by the review
     * settings page.
     *
     * @param stdClass $grade The grade
     * @return int
     ...
     *
     * @param stdClass $grade The review_grades object from the db
     * @return array
     */
    public function files_for_gradebook(stdClass $grade) : array {
        return [
            'contextid' => $this->review->get_context()->id,
            'component' => REVIEWFEEDBACK_COMMENTS_COMPONENT,
            'filearea' => REVIEWFEEDBACK_COMMENTS_FILEAREA,
            'itemid' => $grade->id
        ];
    }
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('reviewfeedback_comments',
    ...
    public function get_file_areas() {
        return array(REVIEWFEEDBACK_COMMENTS_FILEAREA => $this->get_name());
    }
    ...
    public function get_external_parameters() {
        ...
        return array('reviewfeedbackcomments_editor' => $editorstructure);
    }
