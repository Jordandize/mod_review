 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class reviewfeedback_offline_grade_importer {
    ...
    /** @var review $review - the review class */
    private $review;
     ...
     */
    public function __construct($importid, review $review, $encoding = 'utf-8', $separator = 'comma') {
    ...
    public function parsecsv($csvdata) {
        $this->csvreader = new csv_import_reader($this->importid, 'reviewfeedback_offline');
    ...
    public function init() {
        ...
        $strid = get_string('recordid', 'review');
        $strmodified = get_string('lastmodifiedgrade', 'review');
        ...
        $this->validusers = $this->review->list_participants($groupid, false);
        return true;
    }
    ...
    public function next() {
        ...
        while ($record = $this->csvreader->next()) {
            ...
            $id = substr($idstr, strlen(get_string('hiddenuser', 'review')));
            if ($userid = $this->review->get_user_id_for_uniqueid($id)) {
