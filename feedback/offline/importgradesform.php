 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class reviewfeedback_offline_import_grades_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $scaleoptions = null;
        if ($review->get_instance()->grade < 0) {
            if ($scale = $DB->get_record('scale', array('id'=>-($review->get_instance()->grade)))) {
        ...
        $mform->addElement('header', 'importgrades', get_string('importgrades', 'reviewfeedback_offline'));
        ...
        while ($record = $gradeimporter->next()) {
            ...
            if ($review->is_blind_marking()) {
                $userdesc = get_string('hiddenuser', 'review') . $review->get_uniqueid_for_user($user->id);
            }

            $usergrade = $review->get_user_grade($user->id, false);
            ...
            if ($usergrade && $usergrade->grade == $grade) {
                ...
                $skip = true;
            } else if ($review->grading_disabled($user->id)) {
                // Skip grade is locked.
                $skip = true;
            } else if (($review->get_instance()->grade > -1) &&
                      (($grade < 0) || ($grade > $review->get_instance()->grade))) {
                // Out of range.
                $skip = true;
            }
            ...
            if (!$skip) {
                ...
                $updates[] = get_string('gradeupdate', 'reviewfeedback_offline',
            ...
            if ($ignoremodified || !$stalemodificationdate) {
                foreach ($record->feedback as $feedback) {
                    ...
                    if ($newvalue != $oldvalue) {
                        $update = true;
                        $updates[] = get_string('feedbackupdate', 'reviewfeedback_offline',
        ...
        if ($update) {
            ...
            $mform->addElement('html', get_string('nochanges', 'reviewfeedback_offline'));
        }
