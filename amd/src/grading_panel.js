 ...
 *
 * @module     mod_review/grading_panel
 * @package    mod_review
 * @class      GradingPanel
...
define(['jquery', 'core/yui', 'core/notification', 'core/templates', 'core/fragment',
        'core/ajax', 'core/str', 'mod_review/grading_form_change_checker',
        'mod_review/grading_events', 'core/event'],
    ...
    GradingPanel.prototype._submitForm = function(event, nextUserId, nextUser) {
        ...
        var reviewid = this._region.attr('data-reviewid');

        // Now we can continue...
        ajax.call([{
            methodname: 'mod_review_submit_grading_form',
            args: {reviewid: reviewid, userid: this._lastUserId, jsonformdata: JSON.stringify(data)},
    ...
    GradingPanel.prototype._handleFormSubmissionResponse = function(formdata, nextUserId, nextUser, response) {
        ...
        if (response.length) {
            ...
            str.get_strings([
                ...
                {key: 'gradechangessaveddetail', component: 'mod_review'},
            ]).done(function(strs) {
    ...
    GradingPanel.prototype._chooseAttempt = function(e) {
        ...
        str.get_strings([
            {key: 'viewadifferentattempt', component: 'mod_review'},
    ...
    GradingPanel.prototype._addPopoutButtons = function(selector) {
        ...
        templates.render('mod_review/popout_button', {}).done(function(html) {
    ...
    GradingPanel.prototype._refreshGradingPanel = function(event, userid, submissiondata, attemptnumber) {
        ...
        window.M.util.js_pending('mod-review-loading-user');
        // First insert the loading template.
        templates.render('mod_review/loading', {}).done(function(html, js) {
            // Update the page.
            this._niceReplaceNodeContents(this._region, html, js).done(function() {
                if (userid > 0) {
                    ...
                    fragment.loadFragment('mod_review', 'gradingpanel', contextid, params).done(function(html, js) {
                        ...
                        .done(function() {
                            ...
                            window.M.util.js_complete('mod-review-loading-user');
                        }.bind(this))
                        .fail(notification.exception);
                    }.bind(this)).fail(notification.exception);
