...
/**
 * This file contains the definition for the renderable classes for the review
 *
 * @package   mod_review
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_submit_for_grading_page implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_gradingmessage implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_form implements renderable {
    /** @var moodleform $form is the edit submission form */
    public $form = null;
    /** @var string $classname is the name of the class to review to the container */
    public $classname = '';
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_user_summary implements renderable {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_feedback_plugin_feedback implements renderable {
    /** @var int SUMMARY */
    const SUMMARY                = 10;
    /** @var int FULL */
    const FULL                   = 20;

    /** @var review_submission_plugin $plugin */
    public $plugin = null;
    /** @var stdClass $grade */
    public $grade = null;
    /** @var string $view */
     ...
     *
     * @param review_feedback_plugin $plugin
     ...
     */
    public function __construct(review_feedback_plugin $plugin,
                                stdClass $grade,
                                $view,
                                $coursemoduleid,
                                $returnaction,
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_submission_plugin_submission implements renderable {
    /** @var int SUMMARY */
    const SUMMARY                = 10;
    /** @var int FULL */
    const FULL                   = 20;
    ...
    /**
     * Constructor
     * @param review_submission_plugin $plugin
     ...
     */
    public function __construct(review_submission_plugin $plugin,
                                stdClass $submission,
                                $view,
                                $coursemoduleid,
                                $returnaction,
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_feedback_status implements renderable {
    ...
    /** @var stdClass review_grade record */
    public $grade = null;
    /** @var int coursemoduleid */
    public $coursemoduleid = 0;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_submission_status implements renderable {
     ...
     * @param array $usergroups - Array containing all groups the user is reviewed to
     */
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_submission_status_compact extends review_submission_status implements renderable {
    // Compact view of the submission status. Not in a table etc.
}

/**
 * Used to output the attempt history for a particular review.
 *
 * @package mod_review
 * @copyright 2012 Davo Smith, Synergy Learning
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_attempt_history implements renderable {
    ...
    public $feedbackplugins = array();
    /** @var int coursemoduleid - The cmid for the review */
    public $coursemoduleid = 0;
...
/**
 * Used to output the attempt history chooser for a particular review.
 *
 * @package mod_review
 * @copyright 2016 Damyon Wiese
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_attempt_history_chooser implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        foreach ($export->submissions as $i => $submission) {
            ...
            $submission->attemptsummary = get_string('attemptheading', 'review', $attemptsummaryparams);
            $submission->statussummary = get_string('submissionstatus_' . $submission->status, 'review');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_header implements renderable {
    /** @var stdClass the review record  */
    public $review = null;
     ...
     * Constructor
     *
     * @param stdClass $review  - the review database record
     ...
     */
    public function __construct(stdClass $review,
                                $context,
                                $showintro,
                                $coursemoduleid,
                                $subpage='',
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_plugin_header implements renderable {
    /** @var review_plugin $plugin */
    public $plugin = null;

    /**
     * Header for a single plugin
     *
     * @param review_plugin $plugin
     */
    public function __construct(review_plugin $plugin) {
        $this->plugin = $plugin;
    }
}
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_grading_summary implements renderable {
    /** @var int participantcount - The number of users who can submit to this review */
    ...
    /** @var int duedate - The review due date (if one is set) */
    public $duedate = 0;
    /** @var int cutoffdate - The review cut off date (if one is set) */
    public $cutoffdate = 0;
    /** @var int coursemoduleid - The review course module id */
    public $coursemoduleid = 0;
    /** @var boolean teamsubmission - Are team submissions enabled for this review */
    ...
    public $cangrade = false;
    /** @var boolean isvisible - Is the review's context module visible to students? */
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_course_index_summary implements renderable {
    /** @var array reviews - A list of course module info and submission counts or statuses */
     ...
     * @param int $timedue - The due date for the review - may be 0 if no duedate
     * @param string $submissioninfo - A string with either the number of submitted reviews, or the
     ...
     */
    public function add_review_info($cmid, $cmname, $sectionname, $timedue, $submissioninfo, $gradeinfo) {
        $this->reviews[] = array('cmid'=>$cmid,
                               'cmname'=>$cmname,
...
/**
 * An review file class that extends rendererable class and is used by the review module.
 *
 * @package   mod_review
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_files implements renderable {
    /** @var context $context */
    public $context;
    /** @var string $context */
    public $dir;
    ...
    public function __construct(context $context, $sid, $filearea, $component) {
        ...
        if (!empty($CFG->enableportfolios)) {
            ...
            if (count($files) >= 1 && !empty($sid) &&
                    has_capability('mod/review:exportownsubmission', $this->context)) {
                $button = new portfolio_add_button();
                $callbackparams = array('cmid' => $this->cm->id,
                                        'sid' => $sid,
                                        'area' => $filearea,
                                        'component' => $component);
                $button->set_callback_options('review_portfolio_caller',
                                              $callbackparams,
                                              'mod_review');
    ...
    public function preprocess($dir, $filearea, $component) {
        ...
        foreach ($dir['files'] as $file) {
            ...
            if (!empty($CFG->enableportfolios)) {
                require_once($CFG->libdir . '/portfoliolib.php');
                $button = new portfolio_add_button();
                if (has_capability('mod/review:exportownsubmission', $this->context)) {
