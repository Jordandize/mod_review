...
/**
 * This file contains the definition for the class review
 *
...
define('REVIEW_SUBMISSION_STATUS_NEW', 'new');
define('REVIEW_SUBMISSION_STATUS_REOPENED', 'reopened');
define('REVIEW_SUBMISSION_STATUS_DRAFT', 'draft');
define('REVIEW_SUBMISSION_STATUS_SUBMITTED', 'submitted');

// Search filters for grading page.
define('REVIEW_FILTER_NONE', 'none');
define('REVIEW_FILTER_SUBMITTED', 'submitted');
define('REVIEW_FILTER_NOT_SUBMITTED', 'notsubmitted');
define('REVIEW_FILTER_SINGLE_USER', 'singleuser');
define('REVIEW_FILTER_REQUIRE_GRADING', 'requiregrading');
define('REVIEW_FILTER_GRANTED_EXTENSION', 'grantedextension');

// Marker filter for grading page.
define('REVIEW_MARKER_FILTER_NO_MARKER', -1);

// Reopen attempt methods.
define('REVIEW_ATTEMPT_REOPEN_METHOD_NONE', 'none');
define('REVIEW_ATTEMPT_REOPEN_METHOD_MANUAL', 'manual');
define('REVIEW_ATTEMPT_REOPEN_METHOD_UNTILPASS', 'untilpass');

// Special value means allow unlimited attempts.
define('REVIEW_UNLIMITED_ATTEMPTS', -1);

// Special value means no grade has been set.
define('REVIEW_GRADE_NOT_SET', -1);

// Grading states.
define('REVIEW_GRADING_STATUS_GRADED', 'graded');
define('REVIEW_GRADING_STATUS_NOT_GRADED', 'notgraded');

// Marking workflow states.
define('REVIEW_MARKING_WORKFLOW_STATE_NOTMARKED', 'notmarked');
define('REVIEW_MARKING_WORKFLOW_STATE_INMARKING', 'inmarking');
define('REVIEW_MARKING_WORKFLOW_STATE_READYFORREVIEW', 'readyforreview');
define('REVIEW_MARKING_WORKFLOW_STATE_INREVIEW', 'inreview');
define('REVIEW_MARKING_WORKFLOW_STATE_READYFORRELEASE', 'readyforrelease');
define('REVIEW_MARKING_WORKFLOW_STATE_RELEASED', 'released');

/** REVIEW_MAX_EVENT_LENGTH = 432000 ; 5 days maximum */
define("REVIEW_MAX_EVENT_LENGTH", "432000");

// Name of file area for intro attachments.
define('REVIEW_INTROATTACHMENT_FILEAREA', 'introattachment');

// Event types.
define('REVIEW_EVENT_TYPE_DUE', 'due');
define('REVIEW_EVENT_TYPE_GRADINGDUE', 'gradingdue');
define('REVIEW_EVENT_TYPE_OPEN', 'open');
define('REVIEW_EVENT_TYPE_CLOSE', 'close');
...
require_once($CFG->dirroot . '/mod/review/feedbackplugin.php');
require_once($CFG->dirroot . '/mod/review/submissionplugin.php');
require_once($CFG->dirroot . '/mod/review/renderable.php');
...
/**
 * Standard base class for mod_review (review types).
 *
 * @package   mod_review
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class review {
    ...
    /** @var stdClass the review record that contains the global settings for this review instance */
    private $instance;

    /** @var grade_item the grade_item record for this review instance's primary grade item. */
    private $gradeitem;

    /** @var context the context of the course module for this review instance
     *               (or just the course if we are creating a new one)
     */
    private $context;

    /** @var stdClass the course this review instance belongs to */
    private $course;

    /** @var stdClass the admin config for all review instances  */
    private $adminconfig;

    /** @var review_renderer the custom renderer for this module */
    private $output;

    /** @var cm_info the course module for this review instance */
    ...
    /** @var array cached list of participants for this review. The cache key will be group, showactive and the context id */
    ...
    /**
     * Constructor for the base review class.
     *
    ...
    public function __construct($coursemodulecontext, $coursemodule, $course) {
        ...
        $this->submissionplugins = $this->load_plugins('reviewsubmission');
        $this->feedbackplugins = $this->load_plugins('reviewfeedback');

        // Extra entropy is required for uniqid() to work on cygwin.
        $this->useridlistid = clean_param(uniqid('', true), PARAM_ALPHANUM);

        if (!isset($SESSION->mod_review_useridlist)) {
            $SESSION->mod_review_useridlist = [];
        }
    }
    ...
    public function register_return_link($action, $params) {
        ...
        if ($cm) {
            $currenturl = new moodle_url('/mod/review/view.php', array('id' => $cm->id));
        } else {
            $currenturl = new moodle_url('/mod/review/index.php', array('id' => $this->get_course()->id));
        }
    ...
    /**
     * Based on the current review settings should we display the intro.
     *
     ...
     *
     * This just checks the review settings. Remember to check
    ...
    /**
     * Does an review have submission(s) or grade(s) already?
     *
     * @return bool
     */
     ...
     *
     * @param string $subtype reviewsubmission | reviewfeedback
     * @param string $type
     * @return mixed review_plugin|null
     */
    public function get_plugin_by_type($subtype, $type) {
        $shortsubtype = substr($subtype, strlen('review'));
     ...
     * @return mixed review_feedback_plugin|null
     */
    public function get_feedback_plugin_by_type($type) {
        return $this->get_plugin_by_type('reviewfeedback', $type);
    }
     ...
     *
     * @param string $type - The type of plugin e.g comments
     * @return mixed review_submission_plugin|null
     */
    public function get_submission_plugin_by_type($type) {
        return $this->get_plugin_by_type('reviewsubmission', $type);
    }
    ...
    public function load_plugins($subtype) {
        ...
        foreach ($names as $name => $path) {
            if (file_exists($path . '/locallib.php')) {
                ...
                $shortsubtype = substr($subtype, strlen('review'));
                $pluginclass = 'review_' . $shortsubtype . '_' . $name;

                $plugin = new $pluginclass($this, $name);

                if ($plugin instanceof review_plugin) {
    ...
    /**
     * Display the review, used by view.php
     *
     * The review is displayed differently depending on your role,
     * the settings for the review and the status of the review.
     *
    ...
    public function view($action='', $args = array()) {
        ...
        if (!empty($action)) {
            $PAGE->set_pagetype('mod-review-' . $action);
        }
        // Now show the right view page.
        if ($action == 'redirect') {
            $nextpageurl = new moodle_url('/mod/review/view.php', $nextpageparams);
            redirect($nextpageurl);
            return;
        } else if ($action == 'savegradingresult') {
            $message = get_string('gradingchangessaved', 'review');
        ...
        } else if ($action == 'viewpluginreviewfeedback') {
            $o .= $this->view_plugin_content('reviewfeedback');
        } else if ($action == 'viewpluginreviewsubmission') {
            $o .= $this->view_plugin_content('reviewsubmission');
            ...
            $o .= $this->view_error_page(get_string('submitforgrading', 'review'), $notices);
     ...
     *             when upgrading an old review to a new one (the plugins get called manually)
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        $update->attemptreopenmethod = REVIEW_ATTEMPT_REOPEN_METHOD_NONE;
        ...
        $returnid = $DB->insert_record('review', $update);
        $this->instance = $DB->get_record('review', array('id'=>$returnid), '*', MUST_EXIST);
        ...
        if ($callplugins) {
            ...
            if (!empty($formdata->completionexpected)) {
                \core_completion\api::update_completion_date_event($formdata->coursemodule, 'review', $this->instance,
        ...
        $DB->update_record('review', $update);

        return $returnid;
    }

    /**
     * Delete all grades from the gradebook for this review.
     *
     * @return bool
     */
    protected function delete_grades() {
        ...
        $result = grade_update('mod/review',
                               $this->get_course()->id,
                               'mod',
                               'review',
    ...
    public function delete_instance() {
        ...
        // Delete files associated with this review.
        $fs = get_file_storage();
        ...
        $DB->delete_records('review_submission', array('review' => $this->get_instance()->id));
        $DB->delete_records('review_grades', array('review' => $this->get_instance()->id));
        $DB->delete_records('review_plugin_config', array('review' => $this->get_instance()->id));
        $DB->delete_records('review_user_flags', array('review' => $this->get_instance()->id));
        $DB->delete_records('review_user_mapping', array('review' => $this->get_instance()->id));
        ...
        $DB->delete_records('review', array('id'=>$this->get_instance()->id));

        return $result;
    }

    /**
     * Deletes a review override from the database and clears any corresponding calendar events
     *
    ...
    public function delete_override($overrideid) {
        ...
        if (empty($cm)) {
            ...
            $cm = get_coursemodule_from_instance('review', $instance->id, $instance->course);
        }

        $override = $DB->get_record('review_overrides', array('id' => $overrideid), '*', MUST_EXIST);

        // Delete the events.
        $conds = array('modulename' => 'review', 'instance' => $this->get_instance()->id);
        ...
        $DB->delete_records('review_overrides', array('id' => $overrideid));
        ...
        if (!empty($override->userid)) {
            ...
            $event = \mod_review\event\user_override_deleted::create($params);
        } else {
            $params['other']['groupid'] = $override->groupid;
            $event = \mod_review\event\group_override_deleted::create($params);
        }

        // Trigger the override deleted event.
        $event->add_record_snapshot('review_overrides', $override);
        $event->trigger();
    ...
    /**
     * Deletes all review overrides from the database and clears any corresponding calendar events
     */
    public function delete_all_overrides() {
        global $DB;

        $overrides = $DB->get_records('review_overrides', array('reviewid' => $this->get_instance()->id), 'id');
    ...
    /**
     * Updates the review properties with override information for a user.
     *
     * Algorithm:  For each review setting, if there is a matching user-specific override,
     *   then use that otherwise, if there are group-specific overrides, return the most
     *   lenient combination of them.  If neither applies, leave the review setting unchanged.
     *
    ...
    /**
     * Returns whether an review has any overrides.
     *
    ...
    public function has_overrides() {
        ...
        $override = $DB->record_exists('review_overrides', array('reviewid' => $this->get_instance()->id));
     ...
     *   lowest sort order. If neither applies, leave the review setting unchanged.
     *
     * @param int $userid The userid.
    ...
    public function override_exists($userid) {
        ...
        $getuseroverride = function($userid) use ($DB) {
            $useroverride = $DB->get_record('review_overrides', ['reviewid' => $this->get_instance()->id, 'userid' => $userid]);
        ...
        $getgroupoverride = function($userid) use ($DB) {
            ...
            $sql = "SELECT * FROM {review_overrides}
                    WHERE groupid $extra AND reviewid = ? ORDER BY sortorder ASC";
    ...
    public function is_override_calendar_event(\calendar_event $event) {
        ...
        if ($DB->get_record('review_overrides', $overrideparams)) {
            return true;
        } else {
            return false;
        }
    ...
    function get_valid_calendar_event_timestart_range(\calendar_event $event) {
        ...
        if ($event->eventtype == REVIEW_EVENT_TYPE_DUE) {
            ...
            if ($submissionsfromdate) {
                $mindate = [
                    $submissionsfromdate,
                    get_string('duedatevalidation', 'review'),
                ];
            }
            ...
            if ($cutoffdate) {
                $maxdate = [
                    $cutoffdate,
                    get_string('cutoffdatevalidation', 'review'),
                ];
            }
            ...
            if ($gradingduedate) {
                ...
                if (!$cutoffdate || $gradingduedate < $cutoffdate) {
                    $maxdate = [
                        $gradingduedate,
                        get_string('gradingdueduedatevalidation', 'review'),
                    ];
                }
            }
            ...
            if ($duedate) {
                ...
                $mindate = [
                    $submissionsfromdate,
                    get_string('gradingduefromdatevalidation', 'review'),
                ];
            }
        }
     ...
     * review submissions for course $data->courseid.
     *
    ...
    public function reset_userdata($data) {
        ...
        $componentstr = get_string('modulenameplural', 'review');
        $status = array();

        $fs = get_file_storage();
        if (!empty($data->reset_review_submissions)) {
            // Delete files associated with this review.
            foreach ($this->submissionplugins as $plugin) {
                $fileareas = array();
                ...
                if (!$plugin->delete_instance()) {
                    $status[] = array('component'=>$componentstr,
                                      'item'=>get_string('deleteallsubmissions', 'review'),
            ...
            $reviewids = $DB->get_records('review', array('course' => $data->courseid), '', 'id');
            list($sql, $params) = $DB->get_in_or_equal(array_keys($reviewids));

            $DB->delete_records_select('review_submission', "review $sql", $params);
            $DB->delete_records_select('review_user_flags', "review $sql", $params);
            ...
            if (!empty($data->reset_gradebook_grades)) {
                $DB->delete_records_select('review_grades', "review $sql", $params);
                // Remove all grades from gradebook.
                require_once($CFG->dirroot.'/mod/review/lib.php');
                review_reset_gradebook($data->courseid);
            }

            // Reset revealidentities for review if blindmarking is enabled.
            if ($this->get_instance()->blindmarking) {
                $DB->set_field('review', 'revealidentities', 0, array('id' => $this->get_instance()->id));
            }
        }

        // Remove user overrides.
        if (!empty($data->reset_review_user_overrides)) {
            $DB->delete_records_select('review_overrides',
                'reviewid IN (SELECT id FROM {review} WHERE course = ?) AND userid IS NOT NULL', array($data->courseid));
            $status[] = array(
                'component' => $componentstr,
                'item' => get_string('useroverridesdeleted', 'review'),
                'error' => false);
        }
        // Remove group overrides.
        if (!empty($data->reset_review_group_overrides)) {
            $DB->delete_records_select('review_overrides',
                'reviewid IN (SELECT id FROM {review} WHERE course = ?) AND groupid IS NOT NULL', array($data->courseid));
            $status[] = array(
                'component' => $componentstr,
                'item' => get_string('groupoverridesdeleted', 'review'),
                'error' => false);
        }

        // Updating dates - shift may be negative too.
        if ($data->timeshift) {
            $DB->execute("UPDATE {review_overrides}
                         SET allowsubmissionsfromdate = allowsubmissionsfromdate + ?
                       WHERE reviewid = ? AND allowsubmissionsfromdate <> 0",
                array($data->timeshift, $this->get_instance()->id));
            $DB->execute("UPDATE {review_overrides}
                         SET duedate = duedate + ?
                       WHERE reviewid = ? AND duedate <> 0",
                array($data->timeshift, $this->get_instance()->id));
            $DB->execute("UPDATE {review_overrides}
                         SET cutoffdate = cutoffdate + ?
                       WHERE reviewid =? AND cutoffdate <> 0",
                array($data->timeshift, $this->get_instance()->id));

            // Any changes to the list of dates that needs to be rolled should be same during course restore and course reset.
            // See MDL-9367.
            shift_course_mod_dates('review',
     ...
     *
     * @param review_plugin $plugin The plugin to update
     * @param stdClass $formdata The form data
     * @return bool false if an error occurs
     */
    protected function update_plugin_instance(review_plugin $plugin, stdClass $formdata) {
    ...
    /**
     * Update the gradebook information for this review.
     *
     * @param bool $reset If true, will reset all grades in the gradbook for this review
    ...
    public function update_gradebook($reset, $coursemoduleid) {
        ...
        require_once($CFG->dirroot.'/mod/review/lib.php');
        $review = clone $this->get_instance();
        $review->cmidnumber = $coursemoduleid;

        // Set review gradebook feedback plugin status (enabled and visible).
        $review->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        ...
        return review_grade_item_update($review, $param);
    }
     ...
     * @return integer
     */
    public function get_review_perpage() {
        $perpage = (int) get_user_preferences('review_perpage', 10);
        $adminconfig = $this->get_admin_config();
        $maxperpage = -1;
    ...
    public function get_admin_config() {
        ...
        $this->adminconfig = get_config('review');
        return $this->adminconfig;
    }

    /**
     * Update the calendar entries for this review.
     *
    ...
    public function update_calendar($coursemoduleid) {
        ...
        if ($instance->duedate) {
            $event->name = get_string('calendardue', 'review', $instance->name);
            ...
            $select = "modulename = :modulename
                       ...
                       AND groupid = 0
                       AND courseid <> 0";
            $params = array('modulename' => 'review', 'instance' => $instance->id, 'eventtype' => $eventtype);
            ...
            $DB->delete_records('event', array('modulename' => 'review', 'instance' => $instance->id,
                'eventtype' => $eventtype));
        }

        $eventtype = REVIEW_EVENT_TYPE_GRADINGDUE;
        if ($instance->gradingduedate) {
            $event->name = get_string('calendargradingdue', 'review', $instance->name);
            $event->eventtype = $eventtype;
            $event->timestart = $instance->gradingduedate;
            $event->timesort = $instance->gradingduedate;
            $event->id = $DB->get_field('event', 'id', array('modulename' => 'review',
    ...
    public function update_instance($formdata) {
        ...
        $result = $DB->update_record('review', $update);
        $this->instance = $DB->get_record('review', array('id'=>$update->id), '*', MUST_EXIST);

        $this->save_intro_draft_files($formdata);

        // Load the review so the plugins have access to it.
        ...
        \core_completion\api::update_completion_date_event($this->get_course_module()->id, 'review', $this->instance,
                $completionexpected);
    ...
    protected function save_intro_draft_files($formdata) {
        if (isset($formdata->introattachments)) {
            file_save_draft_area_files($formdata->introattachments, $this->get_context()->id,
                                       'mod_review', REVIEW_INTROATTACHMENT_FILEAREA, 0);
        }
    }
     ...
     *
     * @param review_plugin $plugin The plugin to add the settings from
     ...
     * @return void
     */
    protected function add_plugin_settings(review_plugin $plugin, MoodleQuickForm $mform, & $pluginsenabled) {
        global $CFG;
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform) {
        $mform->addElement('header', 'submissiontypes', get_string('submissiontypes', 'review'));

        $submissionpluginsenabled = array();
        $group = $mform->addGroup(array(), 'submissionplugins', get_string('submissiontypes', 'review'), array(' '), false);
        foreach ($this->submissionplugins as $plugin) {
            $this->add_plugin_settings($plugin, $mform, $submissionpluginsenabled);
        }
        $group->setElements($submissionpluginsenabled);

        $mform->addElement('header', 'feedbacktypes', get_string('feedbacktypes', 'review'));
        $feedbackpluginsenabled = array();
        $group = $mform->addGroup(array(), 'feedbackplugins', get_string('feedbacktypes', 'review'), array(' '), false);
        foreach ($this->feedbackplugins as $plugin) {
     ...
     *
     * @return string the module name (Review)
     */
    protected function get_module_name() {
        if (isset(self::$modulename)) {
            return self::$modulename;
        }
        self::$modulename = get_string('modulename', 'review');
        return self::$modulename;
    }
     ...
     *
     * @return string the module name plural (Reviews)
     */
    protected function get_module_name_plural() {
        if (isset(self::$modulenameplural)) {
            return self::$modulenameplural;
        }
        self::$modulenameplural = get_string('modulenameplural', 'review');
        return self::$modulenameplural;
    }

    /**
     * Has this review been constructed from an instance?
     *
     * @return bool
     */
    ...
    /**
     * Get the settings for the current instance of this review
     *
    ...
    public function get_instance() {
        ...
        if ($this->get_course_module()) {
            ...
            $this->instance = $DB->get_record('review', $params, '*', MUST_EXIST);
        }
        if (!$this->instance) {
            throw new coding_exception('Improper use of the review class. ' .
                                       'Cannot load the review record.');
        }
        return $this->instance;
    }

    /**
     * Get the primary grade item for this review instance.
     *
    ...
    protected function count_attachments() {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->get_context()->id, 'mod_review', REVIEW_INTROATTACHMENT_FILEAREA,
                        0, 'id', false);
    ...
    /**
     * Get the submission status/grading status for all submissions in this review for the
     * given paticipants.
     *
     * These statuses match the available filters (requiregrading, submitted, notsubmitted, grantedextension).
     * If this is a group review, group info is also returned.
     *
    ...
    private function get_submission_info_for_participants($participants) {
        ...
        $reviewid = $this->get_instance()->id;
        $params['reviewid1'] = $reviewid;
        $params['reviewid2'] = $reviewid;
        $params['reviewid3'] = $reviewid;
        ...
        $from = ' FROM {user} u
                         LEFT JOIN {review_submission} s
                                ON u.id = s.userid
                               AND s.review = :reviewid1
                               AND s.latest = 1
                         LEFT JOIN {review_grades} g
                                ON u.id = g.userid
                               AND g.review = :reviewid2
                               AND g.attemptnumber = s.attemptnumber
                         LEFT JOIN {review_user_flags} uf
                                ON u.id = uf.userid
                               AND uf.review = :reviewid3
            ';
        $where = ' WHERE u.id ' . $insql;

        if (!empty($this->get_instance()->blindmarking)) {
            $from .= 'LEFT JOIN {review_user_mapping} um
                             ON u.id = um.userid
                            AND um.review = :reviewid4 ';
            $params['reviewid4'] = $reviewid;
        ...
        foreach ($records as $userid => $submissioninfo) {
            ...
            if (!empty($submissioninfo->stime) && $submissioninfo->status == REVIEW_SUBMISSION_STATUS_SUBMITTED) {
                $submitted = true;
            }
    ...
    /**
     * Get the submission status/grading status for all submissions in this review.
     * These statuses match the available filters (requiregrading, submitted, notsubmitted, grantedextension).
     * If this is a group review, group info is also returned.
     *
    ...
    private function get_grading_sort_sql() {
        $usersort = flexible_table::get_sort_for_table('mod_review_grading');
    ...
    public function list_participants($currentgroup, $idsonly, $tablesort = false) {
        ...
        if (!isset($this->participants[$key])) {
            list($esql, $params) = get_enrolled_sql($this->context, 'mod/review:submit', $currentgroup,
            ...
            if (!empty($instance->blindmarking)) {
                $additionaljoins .= " LEFT JOIN {review_user_mapping} um
                                  ON u.id = um.userid
                                 AND um.review = :reviewid1
                           LEFT JOIN {review_submission} s
                                  ON u.id = s.userid
                                 AND s.review = :reviewid2
                                 AND s.latest = 1
                        ";
                $params['reviewid1'] = (int) $instance->id;
                $params['reviewid2'] = (int) $instance->id;
                $fields .= ', um.id as recordid ';
            ...
            if ($instance->markingworkflow &&
                    ...
                    !has_capability('mod/review:manageallocations', $this->get_context()) &&
                    has_capability('mod/review:grade', $this->get_context())) {

                $additionaljoins .= ' LEFT JOIN {review_user_flags} uf
                                     ON u.id = uf.userid
                                     AND uf.review = :reviewid3';

                $params['reviewid3'] = (int) $instance->id;
    ...
    public function get_participant($userid) {
        ...
        if (!is_enrolled($this->context, $participant, 'mod/review:submit', $this->show_only_active_users())) {
            return null;
        }
    ...
    /**
     * Load a count of valid teams for this review.
     *
    ...
    public function count_submissions_need_grading($currentgroup = null) {
        ...
        if ($this->get_instance()->teamsubmission) {
            // This does not make sense for group review because the submission is shared.
            return 0;
        }
        ...
        list($esql, $params) = get_enrolled_sql($this->get_context(), 'mod/review:submit', $currentgroup, true);

        $params['reviewid'] = $this->get_instance()->id;
    ...
    public function count_submissions($includenew = false) {
        ...
        if (!$includenew) {
            ...
            $params['status'] = REVIEW_SUBMISSION_STATUS_NEW;
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $currentgroup = groups_get_activity_group($this->get_course_module(), true);
            list($esql, $enrolparams) = get_enrolled_sql($this->get_context(), 'mod/review:submit', $currentgroup, true);
    ...
    public function count_submissions_with_status($status, $currentgroup = null) {
        ...
        $params['reviewid'] = $this->get_instance()->id;
        $params['reviewid2'] = $this->get_instance()->id;
    ...
    protected function get_grading_userid_list($cached = false, $useridlistid = '') {
        if ($cached) {
            ...
            if (empty($SESSION->mod_review_useridlist[$useridlistkey])) {
                $SESSION->mod_review_useridlist[$useridlistkey] = $this->get_grading_userid_list(false, '');
            }
            return $SESSION->mod_review_useridlist[$useridlistkey];
        }
        $filter = get_user_preferences('review_filter', '');
        $table = new review_grading_table($this, 0, $filter, 0, false);
    ...
    protected function pack_files($filesforzipping) {
        ...
        $tempzip = tempnam($CFG->tempdir . '/', 'review_');
        // Zip files.
    ...
    /**
     * Finds all review notifications that have yet to be mailed out, and mails them.
     *
    ...
    public static function cron() {
        ...
        $timenow   = time();
        $lastcron = $DB->get_field('modules', 'lastcron', array('name' => 'review'));

        // Collect all submissions that require mailing.
        // Submissions are included if all are true:
        //   - The review is visible in the gradebook.
        ...
        $sql = "SELECT g.id as gradeid, a.course, a.name, a.blindmarking, a.revealidentities, a.hidegrader,
                 ...
                 FROM {review} a
                 JOIN {review_grades} g ON g.review = a.id
            LEFT JOIN {review_user_flags} uf ON uf.review = a.id AND uf.userid = g.userid
                 JOIN {course_modules} cm ON cm.course = a.course AND cm.instance = a.id
                 JOIN {modules} md ON md.id = cm.module AND md.name = 'review'
                 JOIN {grade_items} gri ON gri.iteminstance = a.id AND gri.courseid = a.course AND gri.itemmodule = md.name
            LEFT JOIN {review_user_mapping} um ON g.id = um.userid AND um.review = a.id
        ...
        $params = array(
            'yesterday' => $yesterday,
            'today' => $timenow,
            'wfreleased' => REVIEW_MARKING_WORKFLOW_STATE_RELEASED,
        );
        $submissions = $DB->get_records_sql($sql, $params);

        if (!empty($submissions)) {

            mtrace('Processing ' . count($submissions) . ' review submissions ...');
            ...
            foreach ($submissions as $submission) {
                ...
                mtrace("Processing review submission $submission->id ...");
                ...
                if (!$cm->uservisible) {
                    // Hold mail notification for reviews the user cannot access until later.
                    continue;
                }
                ...
                if ($submission->hidegrader && !has_capability('mod/review:showhiddengrader', $contextmodule, $user)) {
                ...
                $flags = $DB->get_record('review_user_flags', array('userid'=>$user->id, 'review'=>$submission->review));
                if ($flags) {
                    $flags->mailed = 1;
                    $DB->update_record('review_user_flags', $flags);
                } else {
                    $flags = new stdClass();
                    $flags->userid = $user->id;
                    $flags->review = $submission->review;
                    $flags->mailed = 1;
            ...
            mtrace('Done processing ' . count($submissions) . ' review submissions');
        ...
        foreach ($newlyavailable as $record) {
            $cm = get_coursemodule_from_instance('review', $record->id, 0, false, MUST_EXIST);
            $context = context_module::instance($cm->id);
    ...
    /**
     * Update user flags for this user in this review.
     *
    ...
    public function update_user_flags($flags) {
        global $DB;
        if ($flags->userid <= 0 || $flags->review <= 0 || $flags->id <= 0) {
            return false;
        }

        $result = $DB->update_record('review_user_flags', $flags);
        return $result;
    }

    /**
     * Update a grade in the grade table for the review and in the gradebook.
     *
     * @param stdClass $grade a grade record keyed on id
     * @param bool $reopenattempt If the attempt reopen method is manual, allow another attempt at this review.
     * @return bool true for success
     */
    public function update_grade($grade, $reopenattempt = false) {
        ...
        $DB->update_record('review_grades', $grade);
        ...
        if ($this->gradebook_item_update(null, $grade)) {
            \mod_review\event\submission_graded::create_from_grade($this, $grade)->trigger();
        }
    ...
    protected function view_grant_extension($mform) {
        global $CFG;
        require_once($CFG->dirroot . '/mod/review/extensionform.php');
        ...
        if (empty($mform)) {
            $mform = new mod_review_extension_form(null, $formparams);
        }
        $mform->set_data($data);
        $header = new review_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('grantextension', 'review'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new review_form('extensionform', $mform));
        $o .= $this->view_footer();
        return $o;
    }
    ...
    /**
     * Get a list of the users in the same group as this user that have not submitted the review.
     *
     * @param int $groupid The id of the group whose members we want or 0 for the default group
     * @param bool $onlyids Whether to retrieve only the user id's
     * @return array The users (possibly id's only)
        ...
        foreach ($members as $id => $member) {
            ...
            if ($submission && $submission->status == REVIEW_SUBMISSION_STATUS_SUBMITTED) {
                ...
                if ($this->is_blind_marking()) {
                    $members[$id]->alias = get_string('hiddenuser', 'review') .
    ...
    public function get_group_submission($userid, $groupid, $create, $attemptnumber=-1) {
        ...
        $params = array('review'=>$this->get_instance()->id, 'groupid'=>$groupid, 'userid'=>0);
        if ($attemptnumber >= 0) {
        ...
        if ($create) {
            ...
            $submission->review = $this->get_instance()->id;
            $submission->userid = 0;
            ...
            if ($attemptnumber == -1) {
                ...
                $result = $DB->get_records('review_submission', $params, 'attemptnumber DESC', 'attemptnumber', 0, 1);
                if ($result) {
            ...
            if ($submission->latest) {
                ...
                $DB->set_field('review_submission', 'latest', 0, $params);
            }
            $submission->status = REVIEW_SUBMISSION_STATUS_NEW;
            $sid = $DB->insert_record('review_submission', $submission);
            return $DB->get_record('review_submission', array('id' => $sid));
        }
        return false;
    }

    /**
     * View a summary listing of all reviews in the current course.
     *
     * @return string
     */
    private function view_course_index() {
        ...
        if (!$cms = get_coursemodules_in_course('review', $course->id, 'm.duedate')) {
        ...
        $courseindexsummary = new review_course_index_summary($usesections, $strsectionname);

        $timenow = time();

        $currentsection = '';
        foreach ($modinfo->instances['review'] as $cm) {
            if (!$cm->uservisible) {
                continue;
            }
            ...
            $timedue = $review->get_instance()->duedate;

            if (has_capability('mod/review:grade', $context)) {
                $submitted = $review->count_submissions_with_status(REVIEW_SUBMISSION_STATUS_SUBMITTED);

            } else if (has_capability('mod/review:submit', $context)) {
                if ($review->get_instance()->teamsubmission) {
                    $usersubmission = $review->get_group_submission($USER->id, 0, false);
                } else {
                    $usersubmission = $review->get_user_submission($USER->id, false);
                }

                if (!empty($usersubmission->status)) {
                    $submitted = get_string('submissionstatus_' . $usersubmission->status, 'review');
                } else {
                    $submitted = get_string('submissionstatus_', 'review');
                }
            }
            $gradinginfo = grade_get_grades($course->id, 'mod', 'review', $cm->instance, $USER->id);
            ...
            $courseindexsummary->add_review_info($cm->id, $cm->get_formatted_name(), $sectionname, $timedue, $submitted, $grade);
    ...
    /**
     * This is used for team reviews to get the group for the specified user.
    ...
    protected function view_plugin_content($pluginsubtype) {
        ...
        $plugintype = required_param('plugin', PARAM_PLUGIN);
        $item = null;
        if ($pluginsubtype == 'reviewsubmission') {
            ...
            $o .= $this->get_renderer()->render(new review_header($this->get_instance(),
                                                              $this->get_context(),
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new review_submission_plugin_submission($plugin,
                                                              $item,
                                                              review_submission_plugin_submission::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing a submission.
            \mod_review\event\submission_viewed::create_from_submission($this, $item)->trigger();
            ...
            $o .= $this->get_renderer()->render(new review_header($this->get_instance(),
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new review_feedback_plugin_feedback($plugin,
                                                              $item,
                                                              review_feedback_plugin_feedback::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing feedback.
            \mod_review\event\feedback_viewed::create_from_grade($this, $item)->trigger();
        }
     ...
     * @param review_plugin $plugin - The review plugin
     */
    public function download_rewrite_pluginfile_urls($text, $user, $plugin) {
        // The groupname prefix for the urls doesn't depend on the group mode of the review instance.
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            if ($submissiongroup) {
                ...
                $groupname = get_string('defaultteam', 'review') . '-';
            }
        }
        ...
        if (!get_user_preferences('review_downloadasfolders', 1)) {
    ...
    public function render_editor_content($filearea, $submissionid, $plugintype, $editor, $component, $shortentext = false) {
        ...
        if ($CFG->enableportfolios && has_capability('mod/review:exportownsubmission', $this->context)) {
            ...
            $portfolioparams = array('cmid' => $this->get_course_module()->id,
                                     ...
                                     'editor' => $editor,
                                     'area'=>$filearea);
            $button->set_callback_options('review_portfolio_caller', $portfolioparams, 'mod_review');
            $fs = get_file_storage();
    ...
    protected function view_savegrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new review_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('savegradingresult', 'review')));
        $gradingresult = new review_gradingmessage(get_string('savegradingresult', 'review'),
                                                   $message,
                                                   $this->get_course_module()->id);
    ...
    protected function view_quickgrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new review_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('quickgradingresult', 'review')));
        $lastpage = optional_param('lastpage', null, PARAM_INT);
        $gradingresult = new review_gradingmessage(get_string('quickgradingresult', 'review'),
                                                   $message,
                                                   $this->get_course_module()->id,
                                                   false,
                                                   $lastpage);
    ...
    public function require_view_group_submission($groupid) {
        if (!$this->can_view_group_submission($groupid)) {
            throw new required_capability_exception($this->context, 'mod/review:viewgrades', 'nopermission', '');
        }
    }
    ...
    /**
     * Throw an error if the permissions to view grades in this review are missing.
     *
     * @throws required_capability_exception
     * @return none
     */
    ...
    /**
     * Does this user have view grade or grade permission for this review?
     *
    ...
    public function can_view_grades($groupid = null) {
        // Permissions check.
        if (!has_any_capability(array('mod/review:viewgrades', 'mod/review:grade'), $this->context)) {
            return false;
        }
    ...
    /**
     * Does this user have grade permission for this review?
     *
    ...
    public function can_grade($user = null) {
        // Permissions check.
        if (!has_capability('mod/review:grade', $this->context, $user)) {
            return false;
        }
    ...
    /**
     * Download a zip file of all review submissions.
     *
     * @param array $userids Array of user ids to download review submissions in a zip file
    ...
    protected function download_submissions($userids = false) {
        ...
        $students = get_enrolled_users($this->context, "mod/review:submit", null, 'u.*', null, null, null,
                        $this->show_only_active_users());
        ...
        foreach ($students as $student) {
            ...
            if ((groups_is_member($groupid, $userid) or !$groupmode or !$groupid)) {
                ...
                if ($this->is_blind_marking()) {
                    $prefix = str_replace('_', ' ', $groupname . get_string('participant', 'review'));
                ...
                if ($submission) {
                    $downloadasfolders = get_user_preferences('review_downloadasfolders', 1);
                    foreach ($this->submissionplugins as $plugin) {
                        if ($plugin->is_enabled() && $plugin->is_visible()) {
                            if ($downloadasfolders) {
                                // Create a folder for each user for each review plugin.
                                ...
                                // Create a single folder for all users of all review plugins.
        ...
        if (count($filesforzipping) == 0) {
            $header = new review_header($this->get_instance(),
                                        ...
                                        '',
                                        $this->get_course_module()->id,
                                        get_string('downloadall', 'review'));
            $result .= $this->get_renderer()->render($header);
            $result .= $this->get_renderer()->notification(get_string('nosubmission', 'review'));
            $url = new moodle_url('/mod/review/view.php', array('id'=>$this->get_course_module()->id,
                                                                    'action'=>'grading'));
            $result .= $this->get_renderer()->continue_button($url);
            $result .= $this->view_footer();
        } else if ($zipfile = $this->pack_files($filesforzipping)) {
            \mod_review\event\all_submissions_downloaded::create_from_review($this)->trigger();
    ...
    public function add_to_log($action = '', $info = '', $url='', $return = false) {
        ...
        if ($return) {
            ...
            debugging('The mod_review add_to_log() function is now deprecated.', DEBUG_DEVELOPER);
            return $args;
        }
    ...
    public function get_renderer() {
        ...
        $this->output = $PAGE->get_renderer('mod_review', null, RENDERER_TARGET_GENERAL);
        return $this->output;
    }
     ...
     *
     * For team reviews there are 2 submissions - the student submission and the team submission
    ...
    public function get_user_submission($userid, $create, $attemptnumber=-1) {
        ...
        $params = array('review'=>$this->get_instance()->id, 'userid'=>$userid, 'groupid'=>0);
        if ($attemptnumber >= 0) {
            $params['attemptnumber'] = $attemptnumber;
        }
    ...
    protected function get_submission($submissionid) {
        ...
        $params = array('review'=>$this->get_instance()->id, 'id'=>$submissionid);
        return $DB->get_record('review_submission', $params, '*', MUST_EXIST);
    }
    ...
    public function get_user_flags($userid, $create) {
        ...
        $params = array('review'=>$this->get_instance()->id, 'userid'=>$userid);

        $flags = $DB->get_record('review_user_flags', $params);
        ...
        if ($create) {
            $flags = new stdClass();
            $flags->review = $this->get_instance()->id;
            $flags->userid = $userid;
            $flags->locked = 0;
            ...
            $fid = $DB->insert_record('review_user_flags', $flags);
            $flags->id = $fid;
            return $flags;
        }
        return false;
    ...
    public function get_user_grade($userid, $create, $attemptnumber=-1) {
        ...
        $grades = $DB->get_records('review_grades', $params, 'attemptnumber DESC', '*', 0, 1);
        ...
        if ($create) {
            $grade = new stdClass();
            $grade->review   = $this->get_instance()->id;
            $grade->userid       = $userid;
            ...
            $gid = $DB->insert_record('review_grades', $grade);
            $grade->id = $gid;
            return $grade;
        }
        return false;
    ...
    protected function get_grade($gradeid) {
        ...
        $params = array('review'=>$this->get_instance()->id, 'id'=>$gradeid);
        return $DB->get_record('review_grades', $params, '*', MUST_EXIST);
    }
    ...
    protected function view_single_grading_panel($args) {
        ...
        // Need submit permission to submit an review.
        require_capability('mod/review:grade', $this->context);
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $submissionstatus = new review_submission_status_compact($instance->allowsubmissionsfromdate,
                                                                     ...
                                                                     $this->get_course_module()->id,
                                                                     $this->get_course()->id,
                                                                     review_submission_status::GRADER_VIEW,
                                                                     $showedit,
                                                                     false,
                                                                     $viewfullnames,
                                                                     $extensionduedate,
        ...
        if ($attemptnumber != -1 && ($attemptnumber + 1) != count($allsubmissions)) {
            ...
            $message = get_string('editingpreviousfeedbackwarning', 'review', $params);
        ...
        $mform = new mod_review_grade_form(null,
                                           $formparams,
                                           'post',
                                           '',
        ...
        $o .= $this->get_renderer()->render(new review_form('gradingform', $mform));

        if (count($allsubmissions) > 1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new review_attempt_history_chooser($allsubmissions,
                                                          $allgrades,
                                                          $this->get_course_module()->id,
                                                          $userid);
        ...
        \mod_review\event\grading_form_viewed::create_from_user($this, $user)->trigger();
    ...
    protected function view_single_grade_page($mform) {
        ...
        $header = new review_header($instance,
                                    $this->get_context(),
                                    false,
                                    $this->get_course_module()->id,
                                    get_string('grading', 'review'));
        $o .= $this->get_renderer()->render($header);
        ...
        if ($user) {
            $this->update_effective_access($userid);
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->get_context());
            $usersummary = new review_user_summary($user,
                                                   $this->get_course()->id,
                                                   $viewfullnames,
                                                   $this->is_blind_marking(),
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $submissionstatus = new review_submission_status($instance->allowsubmissionsfromdate,
                                                             $instance->alwaysshowdescription,
                                                             $submission,
                                                             $instance->teamsubmission,
                                                             $teamsubmission,
        ...
        if (count($allsubmissions) > 1 && $attemptnumber == -1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new review_attempt_history($allsubmissions,
                                                  $allgrades,
                                                  $this->get_submission_plugins(),
    ...
    // KOBA
    protected function view_appoint_reviewers_page($args) {
        // Permissions check.
        if (!has_capability('mod/review:grade', $this->context)) {
            throw new required_capability_exception($this->context, 'mod/review:grade', 'nopermission', '');
        }

        global $DB, $PAGE;
        $reviewid = $this->instance->id;
        $courseid = $this->course->id;

        // Step one - get users
        list($esql, $params) = get_enrolled_sql($this->context, "mod/review:submit");
        $sql = 'SELECT CONCAT(u.id, ".", IFNULL(gm.groupid, "null")) rowkey, u.id, u.firstname, u.lastname, u.picture, u.imagealt, ra.authorid, gm.groupid groupid, gm.groupname groupname
                  FROM ('.$esql.') e
        ...
        // Step two - get currentreviewid
        // !USED IN TEMPLATE
        $currentreviewid = $reviewid;

        // Step three - get all review as possible target review
        $reviews = $DB->get_records("review", array("course" => $courseid), '', "id, name, targetreviewid, enableaccess");
        // !USED IN TEMPLATE
        $reviewsJson = json_encode($reviews);

        // Step four - remember coursemoduleid
        // !USED IN TEMPLATE
...
<body>
    <div class="container mt-3 mb-3">
        <div class="mod_review__assign-reviewers">
            <div class="row">
                <div class="col">
                    <h1 id="reviewName" class="mb-3"></h1>
                </div>
            </div>
            <div class="row">
                <div class="col-4 input-group mb-3">
                    <div class="input-group-prepend">
                        <label class="input-group-text" for="selectGroup">Group</label>
                    </div>
                    <select class="custom-select" id="selectGroup" onchange="onChangeGroup()">
                        <option selected value="all">Any</option>
                    </select>
                </div>
                <div class="col-8 input-group mb-3">
                    <div class="input-group-prepend">
                        <label class="input-group-text" for="selectTargetReview">Target review</label>
                    </div>
                    <select class="custom-select" id="selectTargetReview"">
            ...
            <div class="row">
                <div class="col _controlers">
                    <div class="row d-flex justify-content-center checkbox-row">
                        <div class="custom-control custom-checkbox my-1 mr-sm-2">
                            <input type="checkbox" class="custom-control-input checkbox" id="enableAccess">
                            <label class="custom-control-label checkbox-label" for="enableAccess">
                                Enable access
                            </label>
                        </div>
                    </div>
    ...
    <script>
        const REVIEW_NAME = $("#reviewName");
        const GROUPS_DROPDOWN = $("#selectGroup");
        const TARGET_REVIEWS_DROPDOWN = $("#selectTargetReview");
        const STUDENTS_COLUMN = $("#students");
        const ASSIGNED_COLUMN = $("#assigned");
        const UNASSIGNED_COLUMN = $("#unassigned");
        const ACCESS_CHECKBOX = $("#enableAccess");
        ...
        $(function() {
            ...
            fillTargetReviewsDropdown(allReviews);

            const groupid = GROUPS_DROPDOWN.val();
            const students = getStudentsByGroup(groupid);
            renderColumns(students);

            checkCheckbox();
            bindButtons();
        });

        function renderColumns(students) {
            populateStudents(students);
            populateAssigned(students);
            populateUnassigned(students);

            makeDraggable();
            makeDroppable();

            disableDragNDropInAssignColumn();
        }
        ...
        function makeDroppable() {
            $( ".droppable" ).droppable({
                ...
                drop: function(event, ui) {
                    ...
                    const reviewerid = $drop.data("id");
                    const authorid = $drag.data("id");

                    assignAuthor(reviewerid, authorid);
                    dropAssign($drop, $drag, $parent, reviewerid);
                }
            });
        }

        function disableDragNDropInAssignColumn() {
            ASSIGNED_COLUMN.children().each(function() {
                const assignRow = $(this);
                const isEmpty = !$.trim(assignRow.html());
                if (!isEmpty) {
                    const authorRow = assignRow.children();
                    assignRow.droppable("disable");
                    authorRow.draggable("disable");
                }
            });
        }

        function dropAssign($drop, $drag, $parent, reviewerid) {
            $drag
                .find("._cross")
                    .addClass("_active")
                    .click(function() {
                        cancelAssign($drop, $drag, $parent, reviewerid);
                    })
                    .end()
                .appendTo($drop);
        ...
        function cancelAssign($drop, $drag, $parent, reviewerid) {
            unassignAuthor(reviewerid);
            if ($parent.length) {
                $drag
                    .find("._cross")
                    .removeClass("_active")
        ...
        const currentreviewid = ' . $currentreviewid . ';

        const coursemoduleid = ' . $coursemoduleid . ';

        const ALL_STUDENTS = [];
        Object.values(data).forEach((item) => {
            ALL_STUDENTS.push(item);
        });
        ALL_STUDENTS.sort((a, b) => {
        ...
        function setReviewName() {
            const currentreview = Object.values(allReviews).find((review) => {
                return review.id == currentreviewid;
            });
            REVIEW_NAME.html(currentreview.name);
        }
        ...
        function fillGroupsDropdown(groups) {
            Object.values(groups).forEach((group) => {
                if (group.id != null) {
                    ...
                    GROUPS_DROPDOWN.append(option);
                }
            });
            const nogroup = Object.values(groups).find((group) => {
                return group.id == null;
            });
            if (nogroup) {
                const option = `<option value="${nogroup.id}">Not in group</option>`;
                GROUPS_DROPDOWN.append(option);
            }
        }

        function fillTargetReviewsDropdown(reviews) {
            const currentTargetReviewId = Object.values(allReviews).find((review) => {
                return review.id == currentreviewid;
            }).targetreviewid;
            Object.values(reviews).forEach((review) => {
                if (review.id != currentreviewid) {
                    const option = `<option ${review.id == currentTargetReviewId ? "selected" : ""} value="${review.id}">${review.name}</option>`;
                    TARGET_REVIEWS_DROPDOWN.append(option);
                }
            });
        }

        function populateStudents(students) {
            STUDENTS_COLUMN.empty();
            students.forEach((student) => {
                const studentRow = createStudentRow(student);
                STUDENTS_COLUMN.append(studentRow);
            });
        }

        function populateAssigned(students) {
            ASSIGNED_COLUMN.empty();
            students.forEach((student) => {
                let assignedRow = createAssignedPlaceholder(student);
                if (student.authorid) {
                    assignedRow = $(assignedRow);
                    const author = findStudentById(student.authorid);
                    let authorRow = createAuthorRow(author);
                    authorRow = $(authorRow);
                    authorRow.find("._cross")
                        .addClass("_active")
                        .click(function() {
                            const $unassignedPlaceholder = $(`[data-id=${author.id}]`, UNASSIGNED_COLUMN);
                            cancelAssign(assignedRow, authorRow, $unassignedPlaceholder, student.id);
                        });
                    assignedRow.append(authorRow);
                }
                ASSIGNED_COLUMN.append(assignedRow);

            });
        }

        function populateUnassigned(students) {
            UNASSIGNED_COLUMN.empty();
            const unassignedStudents = students.filter((s1) => {
                return !students.find((s2) => {
                    return s2.authorid == s1.id;
                });
            });
            students.forEach((student) => {
                let unassignedRow = createUnassignedPlaceholder(student);
                const isAssigned = ALL_STUDENTS.find((s1) => {
                    return s1.authorid == student.id;
                });
                if (!isAssigned) {
                    unassignedRow = $(unassignedRow);
                    const authorRow = createAuthorRow(student);
                    unassignedRow.append(authorRow);
                }
                UNASSIGNED_COLUMN.append(unassignedRow);
            });
        }
        ...
        function createStudentRow(student) {
            ...
            return `
                <div class="row _name-row _name ui-state-default">
                    <div>
                        <img src="${student.pictureurl}" class="float-left" alt="${fullName}">
        ...
        function createAssignedPlaceholder(student) {
            return `
                <div class="row _name-row droppable" data-id="${student.id}"></div>
            `;
        }

        function createUnassignedPlaceholder(student) {
            return `
                <div class="row _name-row" data-id="${student.id}"></div>
            `;
        }

        function createAuthorRow(student) {
            const fullName = `${student.firstname} ${student.lastname}`;
            return `
                <div class="draggable _name _highlighted" data-id="${student.id}">
                    <div>
                        <img src="${student.pictureurl}" class="float-left" alt="${fullName}">
                        <div class="fullname">${fullName}</div>
                    </div>
        ...
        function onChangeGroup() {
            const selectedGroup = GROUPS_DROPDOWN.val();
            const students = getStudentsByGroup(selectedGroup);
            renderColumns(students);
        }

        function getStudentsByGroup(groupid) {
            if (groupid === "all") {
                const uniqueStudents = ALL_STUDENTS.reduce((uniques, student) => {
                    const alreadyIn = uniques.find(u => student.id === u.id);
                    if (!alreadyIn) {
                        uniques.push(student);
                    }
                    return uniques;
                }, []);
                return uniqueStudents;
            } else if (groupid === "null") {
                const noGroupStudents = ALL_STUDENTS.filter((student) => {
                    return student.groupid == null;
                })
                return noGroupStudents;
            } else {
                const groupStudents = ALL_STUDENTS.filter((student) => {
        ...
        function assignAuthor(studentid, authorid) {
            ALL_STUDENTS.forEach((student) => {
                if (student.id == studentid) {
                    student.authorid = authorid;
                }
            });
        }

        function unassignAuthor(studentid) {
            ALL_STUDENTS.forEach((student) => {
                if (student.id == studentid) {
                    student.authorid = null;
                }
            });
        }

        function checkCheckbox() {
            const enableaccess = Object.values(allReviews).find((review) => {
                return review.id == currentreviewid;
            }).enableaccess;
            ACCESS_CHECKBOX.prop("checked", enableaccess == 1 ? true : false);
        }

        function bindButtons() {
            const reviewPageUri = `/mod/review/view.php?id=${coursemoduleid}`;
            $("#save").click(() => {
                const pairs = getPairs();
                const targetreviewid = getTargetReviewId();
                const enableaccess = getCheckboxValue();
                $.post(`/mod/review/view.php?id=${coursemoduleid}&action=saveappointreviewers`,
                { pairs, targetreviewid, enableaccess})
                .done((data) => {
                    window.location.href = reviewPageUri;
                })
                .fail((data) => {
                    alert("Error!");
                });
        ...
        function getPairs() {
            ...
            ALL_STUDENTS.forEach((student) => {
                if (student.authorid) {
                    pairs[student.id] = +student.authorid;
                }
            });
            return pairs;
        }
        ...
        function getTargetReviewId() {
            const target = TARGET_REVIEWS_DROPDOWN.val();
            return target == "none" ? 0 : +target;
        }

        function getCheckboxValue() {
            return ACCESS_CHECKBOX.prop("checked") ? 1 : 0;
        }
    </script>
</body>
    ';
    ...
    // KOBA
    protected function post_appoint_reviewers_page($args) {
        // Permissions check.
        if (!has_capability('mod/review:grade', $this->context)) {
            throw new required_capability_exception($this->context, 'mod/review:grade', 'nopermission', '');
        }
        ...
        // First step - update targetreviewid
        $reviewObj = new stdClass();
        $reviewObj->id = $reviewid;
        $reviewObj->targetreviewid = $_POST["targetreviewid"];
        $reviewObj->enableaccess = $_POST["enableaccess"];
        $DB->update_record("review", $reviewObj);

        // Second step - update reviewer<=>author pairs for this review
        $DB->delete_records("review_reviewers_authors", array("review" => $reviewid));

        foreach ($_POST["pairs"] as $key => $value) {
            $pair = new stdClass();
            $pair->review = $reviewid;
            $pair->reviewerid = $key;
            $pair->authorid = $value;
            $DB->insert_record("review_reviewers_authors", $pair);

            $o .= htmlspecialchars($key)."=>".htmlspecialchars($value)."\n";
        }
    ...
    protected function view_remove_submission_confirm() {
        ...
        $urlparams = array('id' => $this->get_course_module()->id,
                           ...
                           'userid' => $userid,
                           'sesskey' => sesskey());
        $confirmurl = new moodle_url('/mod/review/view.php', $urlparams);

        $urlparams = array('id' => $this->get_course_module()->id,
                           'action' => 'view');
        $cancelurl = new moodle_url('/mod/review/view.php', $urlparams);

        if ($userid == $USER->id) {
            $confirmstr = get_string('removesubmissionconfirm', 'review');
        } else {
            $name = $this->fullname($user);
            $confirmstr = get_string('removesubmissionconfirmforstudent', 'review', $name);
        }
        ...
        \mod_review\event\remove_submission_form_viewed::create_from_user($this, $user)->trigger();
    ...
    protected function view_reveal_identities_confirm() {
        require_capability('mod/review:revealidentities', $this->get_context());
        ...
        $o .= $this->get_renderer()->confirm(get_string('revealidentitiesconfirm', 'review'),
                                             $confirmurl,
                                             $cancelurl);
        $o .= $this->view_footer();

        \mod_review\event\reveal_identities_confirmation_page_viewed::create_from_review($this)->trigger();
    ...
    protected function view_return_links() {
        ...
        $url = new moodle_url('/mod/review/view.php', $params);
        return $this->get_renderer()->single_button($url, get_string('back'), 'get');
    }

    /**
     * View the grading table of all submissions for this review.
     *
     * @return string
     */
    protected function view_grading_table() {
        ...
        require_once($CFG->dirroot . '/mod/review/gradingoptionsform.php');
        require_once($CFG->dirroot . '/mod/review/quickgradingform.php');
        require_once($CFG->dirroot . '/mod/review/gradingbatchoperationsform.php');
        $o = '';
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
            ...
            $links[$gradebookurl] = get_string('viewgradebook', 'review');
        }
        if ($this->is_any_submission_plugin_enabled() && $this->count_submissions()) {
            $downloadurl = '/mod/review/view.php?id=' . $cmid . '&action=downloadall';
            $links[$downloadurl] = get_string('downloadall', 'review');
        }
        if ($this->is_blind_marking() &&
                has_capability('mod/review:revealidentities', $this->get_context())) {
            $revealidentitiesurl = '/mod/review/view.php?id=' . $cmid . '&action=revealidentities';
            $links[$revealidentitiesurl] = get_string('revealidentities', 'review');
        }
        foreach ($this->get_feedback_plugins() as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                foreach ($plugin->get_grading_actions() as $action => $description) {
                    $url = '/mod/review/view.php' .
                           '?id=' .  $cmid .
        ...
        $gradingactions->set_label(get_string('choosegradingaction', 'review'));

        $gradingmanager = get_grading_manager($this->get_context(), 'mod_review', 'submissions');

        $perpage = $this->get_review_perpage();
        $filter = get_user_preferences('review_filter', '');
        $markerfilter = get_user_preferences('review_markerfilter', '');
        $workflowfilter = get_user_preferences('review_workflowfilter', '');
        $controller = $gradingmanager->get_active_controller();
        $showquickgrading = empty($controller) && $this->can_grade();
        $quickgrading = get_user_preferences('review_quickgrading', false);
        $showonlyactiveenrolopt = has_capability('moodle/course:viewsuspendedusers', $this->context);
        $downloadasfolders = get_user_preferences('review_downloadasfolders', 1);

        $markingallocation = $this->get_instance()->markingworkflow &&
            $this->get_instance()->markingallocation &&
            has_capability('mod/review:manageallocations', $this->context);
        // Get markers to use in drop lists.
        $markingallocationoptions = array();
        if ($markingallocation) {
            list($sort, $params) = users_order_by_sql('u');
            // Only enrolled users could be reviewed as potential markers.
            $markers = get_enrolled_users($this->context, 'mod/review:grade', 0, 'u.*', $sort);
            $markingallocationoptions[''] = get_string('filternone', 'review');
            $markingallocationoptions[REVIEW_MARKER_FILTER_NO_MARKER] = get_string('markerfilternomarker', 'review');
        ...
        $gradingoptionsform = new mod_review_grading_options_form(null,
        ...
        $gradingbatchoperationsform = new mod_review_grading_batch_operations_form(null,
                                                                                   $batchformparams,
                                                                                   'post',
                                                                                   '',
                                                                                   $classoptions);
        ...
        if ($this->is_blind_marking() && has_capability('mod/review:viewblinddetails', $this->get_context())) {
            $o .= $this->get_renderer()->notification(get_string('blindmarkingenabledwarning', 'review'), 'notifymessage');
        }

        // Load and print the table of submissions.
        if ($showquickgrading && $quickgrading) {
            $gradingtable = new review_grading_table($this, $perpage, $filter, 0, true);
            ...
            $quickformparams = array('cm'=>$this->get_course_module()->id,
                                     ...
                                     'page' => $page);
            $quickgradingform = new mod_review_quick_grading_form(null, $quickformparams);

            $o .= $this->get_renderer()->render(new review_form('quickgradingform', $quickgradingform));
        } else {
            $gradingtable = new review_grading_table($this, $perpage, $filter, 0, false);
        ...
        if ($this->can_grade()) {
            ...
            $SESSION->mod_review_useridlist[$this->get_useridlist_key()] = $useridlist;
        }
        ...
        if (count($users) != 0 && $this->can_grade()) {
            ...
            $reviewform = new review_form('gradingbatchoperationsform', $gradingbatchoperationsform);
            $o .= $this->get_renderer()->render($reviewform);
        }
        $reviewform = new review_form('gradingoptionsform',
                                      $gradingoptionsform,
                                      'M.mod_review.init_grading_options');
        $o .= $this->get_renderer()->render($reviewform);
        return $o;
    }
    ...
    protected function view_grader() {
        ...
        $args = [
            ...
            'subpage' => get_string('grading', 'review')
        ];
        ...
        \mod_review\event\grading_table_viewed::create_from_review($this)->trigger();
    ...
    /**
     * Message for students when review submissions have been closed.
     *
    ...
    protected function view_notices($title, $notices) {
        ...
        $url = new moodle_url('/mod/review/view.php', array('id'=>$this->get_course_module()->id, 'action'=>'view'));
    ...
    public function fullname($user) {
        if ($this->is_blind_marking()) {
            $hasviewblind = has_capability('mod/review:viewblinddetails', $this->get_context());
            ...
            if ($hasviewblind) {
                return get_string('participant', 'review') . ' ' . $uniqueid . ' (' .
    ...
    protected function view_edit_submission_page($mform, $notices) {
        ...
        $o = '';
        require_once($CFG->dirroot . '/mod/review/submission_form.php');
        // Need submit permission to submit an review.
        $userid = optional_param('userid', $USER->id, PARAM_INT);
        $user = $DB->get_record('user', array('id'=>$userid), '*', MUST_EXIST);
        ...
        if ($userid == $USER->id) {
            ...
            require_capability('mod/review:submit', $this->context);
            ...
            $name = $this->fullname($user);
            $title = get_string('editsubmissionother', 'review', $name);
        }

        if (!$this->submissions_open($userid)) {
            $message = array(get_string('submissionsclosed', 'review'));
            return $this->view_notices($title, $message);
        }

        $postfix = '';
        if ($this->has_visible_attachments()) {
            $postfix = $this->render_area_files('mod_review', REVIEW_INTROATTACHMENT_FILEAREA, 0);
        }
        $o .= $this->get_renderer()->render(new review_header($this->get_instance(),
                                                      $this->get_context(),
                                                      $this->show_intro(),
        ...
        $data->userid = $userid;
        if (!$mform) {
            $mform = new mod_review_submission_form(null, array($this, $data));
        }
        ...
        $o .= $this->get_renderer()->render(new review_form('editsubmissionform', $mform));

        $o .= $this->view_footer();

        \mod_review\event\submission_form_viewed::create_from_user($this, $user)->trigger();

        return $o;
    }

    /**
     * See if this review has a grade yet.
     *
     * @param int $userid
     * @return bool
     */
    ...
    public function can_view_submission($userid) {
        ...
        if ($userid == $USER->id && has_capability('mod/review:submit', $this->context)) {
            return true;
        }
    ...
    // KOBA
    public function can_download_submission_as_reviewer($userid) {
        global $USER, $DB;
        $sql = "SELECT *
                  FROM {review} r
                  JOIN {review_reviewers_authors} ra ON r.id = ra.review
                 WHERE r.targetreviewid = :targetreviewid
                       AND
                       ra.reviewerid = :reviewerid
                       AND
                       ra.authorid = :authorid";
        $params = array(
            "targetreviewid" => $this->get_instance()->id,
            "reviewerid" => $USER->id,
            "authorid" => $userid
        );
    ...
    protected function process_grading_batch_operation(& $mform) {
        ...
        $mform = new mod_review_grading_batch_operations_form(null,
                                                              $batchformparams,
                                                              'post',
                                                              '',
                                                              $formclasses);
    ...
    protected function view_batch_set_workflow_state($mform) {
        ...
        require_once($CFG->dirroot . '/mod/review/batchsetmarkingworkflowstateform.php');
        ...
        foreach ($userlist as $userid) {
            ...
            $usershtml .= $this->get_renderer()->render(new review_user_summary($user,
                                                                $this->get_course()->id,
                                                                $viewfullnames,
                                                                $this->is_blind_marking(),
                                                                $this->get_uniqueid_for_user($user->id),
        ...
        $mform = new mod_review_batch_set_marking_workflow_state_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new review_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingworkflowstate', 'review'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new review_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_review\event\batch_set_workflow_state_viewed::create_from_review($this)->trigger();
    ...
    public function view_batch_markingallocation($mform) {
        ...
        require_once($CFG->dirroot . '/mod/review/batchsetallocatedmarkerform.php');
        ...
        list($sort, $params) = users_order_by_sql('u');
        // Only enrolled users could be reviewed as potential markers.
        $markers = get_enrolled_users($this->get_context(), 'mod/review:grade', 0, 'u.*', $sort);
        $markerlist = array();
        foreach ($markers as $marker) {
        ...
        $mform = new mod_review_batch_set_allocatedmarker_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new review_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingallocation', 'review'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new review_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_review\event\batch_set_marker_allocation_viewed::create_from_review($this)->trigger();
    ...
    protected function check_submit_for_grading($mform) {
        ...
        require_once($CFG->dirroot . '/mod/review/submissionconfirmform.php');
        ...
        if ($mform == null) {
            $mform = new mod_review_confirm_submission_form(null, array($requiresubmissionstatement,
                                                                        $submissionstatement,
                                                                        $this->get_course_module()->id,
                                                                        $data));
        }
        ...
        $o .= $this->get_renderer()->render(new review_header($this->get_instance(),
                                                              ...
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              get_string('confirmsubmissionheading', 'review')));
        $submitforgradingpage = new review_submit_for_grading_page($notifications,
                                                                   $this->get_course_module()->id,
                                                                   $mform);
        $o .= $this->get_renderer()->render($submitforgradingpage);
        $o .= $this->view_footer();

        \mod_review\event\submission_confirmation_form_viewed::create_from_review($this)->trigger();

        return $o;
    }

    /**
     * Creates an review_submission_status renderable.
     *
     * @param stdClass $user the user to get the report for
     * @param bool $showlinks return plain text or links to the profile
     * @return review_submission_status renderable object
     */
    public function get_review_submission_status_renderable($user, $showlinks) {
        global $PAGE;
        ...
        $gradingmanager = get_grading_manager($this->context, 'mod_review', 'submissions');
        ...
        $submissionstatus = new review_submission_status($instance->allowsubmissionsfromdate,
                                                          ...
                                                          $this->get_course_module()->id,
                                                          $this->get_course()->id,
                                                          review_submission_status::STUDENT_VIEW,
                                                          $showedit,
                                                          $showsubmit,
                                                          $viewfullnames,
                                                          $extensionduedate,
    ...
    /**
     * Creates an review_feedback_status renderable.
     *
     * @param stdClass $user the user to get the report for
     * @return review_feedback_status renderable object
     */
    public function get_review_feedback_status_renderable($user) {
        global $CFG, $DB, $PAGE;
        ...
        if ($this->get_instance()->markingworkflow && $gradingstatus != REVIEW_MARKING_WORKFLOW_STATE_RELEASED) {
            $emptyplugins = true; // Don't show feedback plugins until released either.
        }

        $cangrade = has_capability('mod/review:grade', $this->get_context());
        ...
        if (($hasgrade || !$emptyplugins) && $gradevisible) {
            ...
            if ($grade) {
                \mod_review\event\feedback_viewed::create_from_grade($this, $grade)->trigger();
            }
            $feedbackstatus = new review_feedback_status($gradefordisplay,
                                                  $gradeddate,
                                                  $grader,
                                                  $this->get_feedback_plugins(),
                                                  $grade,
            ...
            $showgradername = (
                    has_capability('mod/review:showhiddengrader', $this->context) or
    ...
    /**
     * Creates an review_attempt_history renderable.
     *
     * @param stdClass $user the user to get the report for
     * @return review_attempt_history renderable object
     */
    public function get_review_attempt_history_renderable($user) {
    ...
    public function view_student_summary($user, $showlinks) {
        ...
        if ($this->can_view_submission($user->id)) {
            ...
            if (has_capability('mod/review:submit', $this->get_context(), $user, false)) {
                $submissionstatus = $this->get_review_submission_status_renderable($user, $showlinks);
                $o .= $this->get_renderer()->render($submissionstatus);
            }

            // If there is a visible grade, show the feedback.
            $feedbackstatus = $this->get_review_feedback_status_renderable($user);
            if ($feedbackstatus) {
                $o .= $this->get_renderer()->render($feedbackstatus);
            }

            // If there is more than one submission, show the history.
            $history = $this->get_review_attempt_history_renderable($user);
    ...
    protected function show_submit_button($submission = null, $teamsubmission = null, $userid = null) {
        if ($teamsubmission) {
            if ($teamsubmission->status === REVIEW_SUBMISSION_STATUS_SUBMITTED) {
                // The review submission has been completed.
                return false;
            } else if ($this->submission_empty($teamsubmission)) {
                // There is nothing to submit yet.
                return false;
            } else if ($submission && $submission->status === REVIEW_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if ($submission->status === REVIEW_SUBMISSION_STATUS_SUBMITTED) {
                // The review submission has been completed.
                return false;
    ...
    protected function get_all_grades($userid) {
        ...
        $grades = $DB->get_records('review_grades', $params, 'attemptnumber ASC');
        ...
        // Show the grader's identity if 'Hide Grader' is disabled or has the 'Show Hidden Grader' capability.
        $showgradername = (
            has_capability('mod/review:showhiddengrader', $this->context, $userid) or
            !$this->is_hidden_grader()
        );
    ...
    public function get_all_submissions($userid) {
        ...
        $submissions = $DB->get_records('review_submission', $params, 'attemptnumber ASC');

        return $submissions;
    }

    /**
     * Creates an review_grading_summary renderable.
     *
     * @param mixed $activitygroup int|null the group for calculating the grading summary (if null the function will determine it)
     * @return review_grading_summary renderable object
     */
    public function get_review_grading_summary_renderable($activitygroup = null) {

        $instance = $this->get_instance();
        $cm = $this->get_course_module();

        $draft = REVIEW_SUBMISSION_STATUS_DRAFT;
        $submitted = REVIEW_SUBMISSION_STATUS_SUBMITTED;
        ...
        if ($instance->teamsubmission) {
            ...
            $summary = new review_grading_summary($this->count_teams($activitygroup),
            ...
            $summary = new review_grading_summary($countparticipants,
                                                  $instance->submissiondrafts,
                                                  $this->count_submissions_with_status($draft, $activitygroup),
                                                  $this->is_any_submission_plugin_enabled(),
                                                  $this->count_submissions_with_status($submitted, $activitygroup),
    ...
    protected function view_submission_page() {
        ...
        $o .= $this->get_renderer()->render(new review_header($instance,
                                                      $this->get_context(),
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      '', '', $postfix));
        ...
        foreach ($plugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                $o .= $this->get_renderer()->render(new review_plugin_header($plugin));
            }
        }

        if ($this->can_view_grades()) {
            // Group selector will only be displayed if necessary.
            $currenturl = new moodle_url('/mod/review/view.php', array('id' => $this->get_course_module()->id));
            $o .= groups_print_activity_menu($this->get_course_module(), $currenturl->out(), true);

            $summary = $this->get_review_grading_summary_renderable();
        ...
        \mod_review\event\submission_status_viewed::create_from_review($this)->trigger();
    ...
    protected function gradebook_item_update($submission=null, $grade=null) {
        ...
        if ($this->get_instance()->markingworkflow && !empty($grade) &&
                $this->get_grading_status($grade->userid) != REVIEW_MARKING_WORKFLOW_STATE_RELEASED) {
        ...
        $review = clone $this->get_instance();
        $review->cmidnumber = $this->get_course_module()->idnumber;
        // Set review gradebook feedback plugin status (enabled and visible).
        $review->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        return review_grade_item_update($review, $gradebookgrade) == GRADE_UPDATE_OK;
    }
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        // Now check the team settings to see if this review qualifies as submitted or draft.
        $team = $this->get_submission_group_members($submission->groupid, true);

        $allsubmitted = true;
        $anysubmitted = false;
        $result = true;
        if ($submission->status != REVIEW_SUBMISSION_STATUS_REOPENED) {
            foreach ($team as $member) {
                ...
                if (!$membersubmission || $membersubmission->status != REVIEW_SUBMISSION_STATUS_SUBMITTED
            ...
            if ($this->get_instance()->requireallteammemberssubmit) {
                ...
                $result = $DB->update_record('review_submission', $submission);
            } else {
                if ($anysubmitted) {
                    $submission->status = REVIEW_SUBMISSION_STATUS_SUBMITTED;
                } else {
            ...
            foreach ($team as $member) {
                ...
                $membersubmission->status = REVIEW_SUBMISSION_STATUS_REOPENED;
                $result = $DB->update_record('review_submission', $membersubmission) && $result;
            }
            $result = $DB->update_record('review_submission', $submission) && $result;
        }
    ...
    /**
     * Is this review open for submissions?
     *
     * Check the due date,
     * prevent late submissions,
     * has this person already submitted,
     * is the review locked?
     *
    ...
    public function submissions_open($userid = 0,
        ...
        if ($submission) {
            ...
            if ($this->get_instance()->submissiondrafts && $submission->status == REVIEW_SUBMISSION_STATUS_SUBMITTED) {
                // Drafts are tracked and the student has submitted the review.
                return false;
            }
        }
    ...
    public function render_area_files($component, $area, $submissionid) {
        ...
        return $this->get_renderer()->review_files($this->context, $submissionid, $area, $component);
    ...
    public function can_edit_submission($userid, $graderid = 0) {
        ...
        if ($userid == $graderid) {
            if ($this->submissions_open($userid) &&
                    has_capability('mod/review:submit', $this->context, $graderid)) {
        ...
        if (!has_capability('mod/review:editothersubmission', $this->context, $graderid)) {
            return false;
        }
    ...
    protected function get_graders($userid) {
        ...
        $potentialgraders = get_enrolled_users($this->context, "mod/review:grade", null, 'u.*', null, null, null, true);
    ...
    protected function get_notifiable_users($userid) {
        ...
        $potentialusers = get_enrolled_users($this->context, "mod/review:receivegradernotifications",
    ...
    protected static function format_notification_message_text($messagetype,
        ...
        $posttext  = format_string($course->shortname, true, $formatparams) .
                     ...
                     $modulename .
                     ' -> ' .
                     format_string($reviewname, true, $formatparams) . "\n";
        $posttext .= '---------------------------------------------------------------------' . "\n";
        $posttext .= get_string($messagetype . 'text', 'review', $info)."\n";
        $posttext .= "\n---------------------------------------------------------------------\n";
        return $posttext;
    }
    ...
    protected static function format_notification_message_html($messagetype,
        ...
        $posthtml  = '<p><font face="sans-serif">' .
                     ...
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/review/index.php?id=' . $course->id . '">' .
                     $modulename .
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/review/view.php?id=' . $coursemodule->id . '">' .
                     format_string($reviewname, true, $formatparams) .
                     '</a></font></p>';
        $posthtml .= '<hr /><font face="sans-serif">';
        $posthtml .= '<p>' . get_string($messagetype . 'html', 'review', $info) . '</p>';
        $posthtml .= '</font><hr />';
        return $posthtml;
    }
     ...
     * @return void
     */
    public static function send_review_notification($userfrom,
                                                        $userto,
                                                        $messagetype,
                                                        $eventtype,
                                                        $updatetime,
        ...
        if ($blindmarking) {
            ...
            $info->username = get_string('participant', 'review') . ' ' . $uniqueidforuser;
            $userfrom->firstname = get_string('participant', 'review');
            $userfrom->lastname = $uniqueidforuser;
            $userfrom->email = $CFG->noreplyaddress;
        } else {
            $info->username = fullname($userfrom, true);
        }
        $info->review = format_string($reviewname, true, array('context'=>$context));
        $info->url = $CFG->wwwroot.'/mod/review/view.php?id='.$coursemodule->id;
        $info->timeupdated = userdate($updatetime, get_string('strftimerecentfull'));

        $postsubject = get_string($messagetype . 'small', 'review', $info);
        ...
        $eventdata->component       = 'mod_review';
        $eventdata->notification    = 1;
        $eventdata->contexturl      = $info->url;
        $eventdata->contexturlname  = $info->review;
        $customdata = [
    ...
    public function send_notification($userfrom, $userto, $messagetype, $eventtype, $updatetime) {
        ...
        self::send_review_notification($userfrom,
                                           $userto,
                                           $messagetype,
                                           $eventtype,
                                           $updatetime,
    ...
    public function submit_for_grading($data, $notices) {
        ...
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('submissionsclosed', 'review');
            return false;
        }
        ...
        if ($submission->status != REVIEW_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if (!empty($data->submissionstatement) && $USER->id == $userid) {
                \mod_review\event\statement_accepted::create_from_submission($this, $submission)->trigger();
            }
            $this->notify_graders($submission);
            $this->notify_student_submission_receipt($submission);

            \mod_review\event\assessable_submitted::create_from_submission($this, $submission, false)->trigger();
    ...
    /**
     * Review submission is processed before grading.
     *
    ...
    public function save_user_extension($userid, $extensionduedate) {
        ...
        // Need submit permission to submit an review.
        require_capability('mod/review:grantextension', $this->context);

        if (!is_enrolled($this->get_course_context(), $userid)) {
            return false;
        }
        if (!has_capability('mod/review:submit', $this->context, $userid)) {
            return false;
        }
        ...
        if ($result) {
            \mod_review\event\extension_granted::create_from_review($this, $userid)->trigger();
        }
        return $result;
    }
    ...
    protected function process_save_quick_grades() {
        ...
        $controller = $gradingmanager->get_active_controller();
        if (!empty($controller)) {
            return get_string('errorquickgradingvsadvancedgrading', 'review');
        }
        ...
        $sql = 'SELECT u.id AS userid, g.grade AS grade, g.timemodified AS lastmodified,
             ...
             LEFT JOIN {review_grades} g ON
                       u.id = g.userid AND
                       g.review = :reviewid2 AND
                       g.attemptnumber = gmx.maxattempt
             LEFT JOIN {review_user_flags} uf ON uf.review = g.review AND uf.userid = g.userid
                 WHERE u.id ' . $userids;
                ...
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if ($plugin->is_quickgrading_modified($modified->userid, $grade)) {
                        if ((int)$current->lastmodified > (int)$modified->lastmodified) {
                            return get_string('errorrecordmodified', 'review');
                        } else {
                            $modifiedusers[$modified->userid] = $modified;
                            continue;
                        }
        ...
        foreach ($modifiedusers as $userid => $modified) {
            ...
            foreach ($this->feedbackplugins as $plugin) {
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if (('reviewfeedback_' . $plugin->get_type()) == $gradebookplugin) {
            ...
            if ($workflowstatemodified || $allocatedmarkermodified) {
                if ($this->update_user_flags($flags) && $workflowstatemodified) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_review\event\workflow_state_updated::create_from_user($this, $user, $flags->workflowstate)->trigger();
                }
            }
            ...
            if ($CFG->enableoutcomes) {
                ...
                if (count($data) > 0) {
                    grade_update_outcomes('mod/review',
                                          $this->course->id,
                                          'mod',
                                          'review',
                                          $this->get_instance()->id,
        ...
        return get_string('quickgradingchangessaved', 'review');
    }
    ...
    public function reveal_identities() {
        ...
        require_capability('mod/review:revealidentities', $this->context);
        ...
        $adminconfig = $this->get_admin_config();
        $gradebookplugin = $adminconfig->feedback_plugin_for_gradebook;
        $gradebookplugin = str_replace('reviewfeedback_', '', $gradebookplugin);
        $grades = $DB->get_records('review_grades', array('review'=>$this->get_instance()->id));
        ...
        \mod_review\event\identities_revealed::create_from_review($this)->trigger();
    }
    ...
    protected function process_save_grading_options() {
        ...
        $gradingoptionsparams = array('cm'=>$this->get_course_module()->id,
                                      ...
                                      'showonlyactiveenrol' => $this->show_only_active_users(),
                                      'downloadasfolders' => get_user_preferences('review_downloadasfolders', 1));
        $mform = new mod_review_grading_options_form(null, $gradingoptionsparams);
        if ($formdata = $mform->get_data()) {
            set_user_preference('review_perpage', $formdata->perpage);
            if (isset($formdata->filter)) {
                set_user_preference('review_filter', $formdata->filter);
            }
            if (isset($formdata->markerfilter)) {
                set_user_preference('review_markerfilter', $formdata->markerfilter);
            }
            if (isset($formdata->workflowfilter)) {
                set_user_preference('review_workflowfilter', $formdata->workflowfilter);
            }
            if ($showquickgrading) {
                set_user_preference('review_quickgrading', isset($formdata->quickgrading));
            }
            if (isset($formdata->downloadasfolders)) {
                set_user_preference('review_downloadasfolders', 1); // Enabled.
            } else {
                set_user_preference('review_downloadasfolders', 0); // Disabled.
            }
    ...
    public function format_grade_for_log(stdClass $grade) {
        ...
        $info = get_string('gradestudent', 'review', array('id'=>$user->id, 'fullname'=>fullname($user)));
        if ($grade->grade != '') {
            $info .= get_string('grade') . ': ' . $this->display_grade($grade->grade, false) . '. ';
        } else {
            $info .= get_string('nograde', 'review');
        }
        return $info;
    }
    ...
    public function format_submission_for_log(stdClass $submission) {
        ...
        if ($submission->userid) {
            ...
            if ($group) {
                ...
                $name = get_string('defaultteam', 'review');
            }
        }
        $status = get_string('submissionstatus_' . $submission->status, 'review');
        $params = array('id'=>$submission->userid, 'fullname'=>$name, 'status'=>$status);
        $info .= get_string('submissionlog', 'review', $params) . ' <br>';
    ...
    /**
     * Copy the current review submission from the last submitted attempt.
     *
     * @param  array $notices Any error messages that should be shown
     *                        to the user at the top of the edit submission form.
     * @return bool
    ...
    public function copy_previous_attempt(&$notices) {
        ...
        if (!$submission || $submission->status != REVIEW_SUBMISSION_STATUS_REOPENED) {
            $notices[] = get_string('submissionnotcopiedinvalidstatus', 'review');
            return false;
        }
        $flags = $this->get_user_flags($USER->id, false);
        ...
        if ($flags && $flags->locked) {
            $notices[] = get_string('submissionslocked', 'review');
            return false;
        }
        ...
        \mod_review\event\submission_duplicated::create_from_submission($this, $submission)->trigger();
        ...
        if (!$instance->submissiondrafts) {
            ...
            // but then they would wonder why there are submitted reviews
            // and they haven't been notified.
            \mod_review\event\assessable_submitted::create_from_submission($this, $submission, true)->trigger();
        }
        return true;
    }
    ...
    /**
     * Save review submission for the current user.
     *
     * @param  stdClass $data
     * @param  array $notices Any error messages that should be shown
     *                        to the user.
    ...
    public function save_submission(stdClass $data, & $notices) {
        ...
        if ($this->new_submission_empty($data)) {
            $notices[] = get_string('submissionempty', 'mod_review');
            return false;
        }
        ...
        if (isset($data->lastmodified) && ($submission->timemodified > $data->lastmodified)) {
            ...
            if ($submission->status !== REVIEW_SUBMISSION_STATUS_NEW) {
                $notices[] = $instance->teamsubmission ? get_string('submissionmodifiedgroup', 'mod_review')
                                                       : get_string('submissionmodified', 'mod_review');
                return false;
            }
        }
        ...
        // Get the flags to check if it is locked.
        if ($flags && $flags->locked) {
            print_error('submissionslocked', 'review');
            return true;
        }
     ...
     * This is specific to the review, marker and student.
     *
     * @param int $userid - The student userid
    ...
    public function add_grade_form_elements(MoodleQuickForm $mform, stdClass $data, $params) {
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                $name = get_string('gradeoutof', 'review', $this->get_instance()->grade);
                if (!$gradingdisabled) {
                    $gradingelement = $mform->addElement('text', 'grade', $name);
                    $mform->addHelpButton('grade', 'gradeoutofhelp', 'review');
                    $mform->setType('grade', PARAM_RAW);
                } else {
                    $strgradelocked = get_string('gradelocked', 'review');
                    $mform->addElement('static', 'gradedisabled', $name, $strgradelocked);
                    $mform->addHelpButton('gradedisabled', 'gradeoutofhelp', 'review');
                }
            } else {
        ...
        if ($this->get_instance()->markingworkflow) {
            ...
            $options = array('' => get_string('markingworkflowstatenotmarked', 'review')) + $states;
            $mform->addElement('select', 'workflowstate', get_string('markingworkflowstate', 'review'), $options);
            $mform->addHelpButton('workflowstate', 'markingworkflowstate', 'review');
            $gradingstatus = $this->get_grading_status($userid);
            if ($gradingstatus != REVIEW_MARKING_WORKFLOW_STATE_RELEASED) {
                if ($grade->grade && $grade->grade != -1) {
                    $reviewgradestring = html_writer::span(grade_floatval($grade->grade), 'currentgrade');
                    $label = get_string('currentreviewgrade', 'review');
                    $mform->addElement('static', 'currentreviewgrade', $label, $reviewgradestring);
                }
            }
        }
        ...
        if ($this->get_instance()->markingworkflow &&
            ...
            // Only enrolled users could be reviewed as potential markers.
            $markers = get_enrolled_users($this->context, 'mod/review:grade', 0, 'u.*', $sort);
            $markerlist = array('' =>  get_string('choosemarker', 'review'));
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->context);
            foreach ($markers as $marker) {
                $markerlist[$marker->id] = fullname($marker, $viewfullnames);
            }
            $mform->addElement('select', 'allocatedmarker', get_string('allocatedmarker', 'review'), $markerlist);
            $mform->addHelpButton('allocatedmarker', 'allocatedmarker', 'review');
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', REVIEW_MARKING_WORKFLOW_STATE_READYFORREVIEW);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', REVIEW_MARKING_WORKFLOW_STATE_INREVIEW);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', REVIEW_MARKING_WORKFLOW_STATE_READYFORRELEASE);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', REVIEW_MARKING_WORKFLOW_STATE_RELEASED);
        }

        $gradestring = '<span class="currentgrade">' . $gradestring . '</span>';
        $mform->addElement('static', 'currentgrade', get_string('currentgrade', 'review'), $gradestring);

        if (count($useridlist) > 1) {
            $strparams = array('current'=>$rownum+1, 'total'=>count($useridlist));
            $name = get_string('outof', 'review', $strparams);
            $mform->addElement('static', 'gradingstudent', get_string('gradingstudent', 'review'), $name);
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $mform->addElement('selectyesno', 'applytoall', get_string('applytoteam', 'review'));
        ...
        if (($attemptnumber == -1 ||
            ...
            $this->get_instance()->attemptreopenmethod != REVIEW_ATTEMPT_REOPEN_METHOD_NONE) {
            $mform->addElement('header', 'attemptsettings', get_string('attemptsettings', 'review'));
            $attemptreopenmethod = get_string('attemptreopenmethod_' . $this->get_instance()->attemptreopenmethod, 'review');
            $mform->addElement('static', 'attemptreopenmethod', get_string('attemptreopenmethod', 'review'), $attemptreopenmethod);
            ...
            if ($maxattempts == REVIEW_UNLIMITED_ATTEMPTS) {
                $maxattempts = get_string('unlimitedattempts', 'review');
            }
            $mform->addelement('static', 'maxattemptslabel', get_string('maxattempts', 'review'), $maxattempts);
            $mform->addelement('static', 'attemptnumberlabel', get_string('attemptnumber', 'review'), $attemptnumber + 1);

            $ismanual = $this->get_instance()->attemptreopenmethod == REVIEW_ATTEMPT_REOPEN_METHOD_MANUAL;
            $issubmission = !empty($submission);
            $isunlimited = $this->get_instance()->maxattempts == REVIEW_UNLIMITED_ATTEMPTS;
            ...
            if ($ismanual && (!$issubmission || $isunlimited || $islessthanmaxattempts)) {
                $mform->addElement('selectyesno', 'addattempt', get_string('addattempt', 'review'));
                $mform->setDefault('addattempt', 0);
            }
        }
        if (!$gradingpanel) {
            $mform->addElement('selectyesno', 'sendstudentnotifications', get_string('sendstudentnotifications', 'review'));
        } else {
            $mform->addElement('hidden', 'sendstudentnotifications', get_string('sendstudentnotifications', 'review'));
            $mform->setType('sendstudentnotifications', PARAM_BOOL);
        }
        // Get review visibility information for student.
        $modinfo = get_fast_modinfo($settings->course, $userid);
        $cm = $modinfo->get_cm($this->get_course_module()->id);

        // Don't allow notification to be sent if the student can't access the review,
        ...
        if (!$cm->uservisible) {
            ...
            $mform->setDefault('sendstudentnotifications', 0);
            if (!$gradingpanel) {
                $mform->disabledIf('sendstudentnotifications', 'workflowstate', 'neq', REVIEW_MARKING_WORKFLOW_STATE_RELEASED);
            }
        } else {
        ...
        if (!$gradingpanel) {
            ...
            $buttonarray = array();
            $name = get_string('savechanges', 'review');
            $buttonarray[] = $mform->createElement('submit', 'savegrade', $name);
            if (!$last) {
                $name = get_string('savenext', 'review');
            ...
            if ($rownum > 0) {
                $name = get_string('previous', 'review');
                $buttonarray[] = $mform->createElement('submit', 'nosaveandprevious', $name);
            }

            if (!$last) {
                $name = get_string('nosavebutnext', 'review');
    ...
    public function remove_submission($userid) {
        ...
        if ($submission->userid != 0) {
            \mod_review\event\submission_status_updated::create_from_submission($this, $submission)->trigger();
        }
        return true;
    }
    ...
    public function lock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_review\event\submission_locked::create_from_user($this, $user)->trigger();
        return true;
    }
    ...
    protected function process_set_batch_marking_workflow_state() {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                ...
                // notified previously, the student can access the review, and the state
                // is "Released".
                $modinfo = get_fast_modinfo($this->course, $userid);
                $cm = $modinfo->get_cm($this->get_course_module()->id);
                if ($formdata->sendstudentnotifications && $cm->uservisible &&
                        $state == REVIEW_MARKING_WORKFLOW_STATE_RELEASED) {
                    $flags->mailed = 0;
                }

                $gradingdisabled = $this->grading_disabled($userid);

                // Will not apply update if user does not have permission to review this workflow state.
                if (!$gradingdisabled && $this->update_user_flags($flags)) {
                    if ($state == REVIEW_MARKING_WORKFLOW_STATE_RELEASED) {
                        // Update Gradebook.
                        $review = clone $this->get_instance();
                        $review->cmidnumber = $this->get_course_module()->idnumber;
                        // Set review gradebook feedback plugin status.
                        $review->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
                        review_update_grades($review, $userid);
                    }

                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_review\event\workflow_state_updated::create_from_user($this, $user, $state)->trigger();
                }
            }
        }
    }
    ...
    protected function process_set_batch_marking_allocation() {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                $flags = $this->get_user_flags($userid, true);
                if ($flags->workflowstate == REVIEW_MARKING_WORKFLOW_STATE_READYFORREVIEW ||
                    $flags->workflowstate == REVIEW_MARKING_WORKFLOW_STATE_INREVIEW ||
                    $flags->workflowstate == REVIEW_MARKING_WORKFLOW_STATE_READYFORRELEASE ||
                    $flags->workflowstate == REVIEW_MARKING_WORKFLOW_STATE_RELEASED) {
                ...
                if ($this->update_user_flags($flags)) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_review\event\marker_updated::create_from_marker($this, $user, $marker)->trigger();
                }
            }
        }
    }
    ...
    public function unlock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_review\event\submission_unlocked::create_from_user($this, $user)->trigger();
        return true;
    }
    ...
    protected function apply_grade_to_user($formdata, $userid, $attemptnumber) {
        ...
        if (!$gradingdisabled) {
            ...
            if (isset($formdata->workflowstate) || isset($formdata->allocatedmarker)) {
                ...
                if ($this->update_user_flags($flags) &&
                    ...
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_review\event\workflow_state_updated::create_from_user($this, $user, $formdata->workflowstate)->trigger();
                }
            }
        }
        $grade->grader= $USER->id;
        ...
        if ($this->get_instance()->markingworkflow &&
                isset($formdata->workflowstate) &&
                $formdata->workflowstate != REVIEW_MARKING_WORKFLOW_STATE_RELEASED) {
    ...
    protected function reopen_submission_if_required($userid, $submission, $addattempt) {
        ...
        $maxattemptsreached = !empty($submission) &&
                              ...
                              $instance->maxattempts != REVIEW_UNLIMITED_ATTEMPTS;
        $shouldreopen = false;
        if ($instance->attemptreopenmethod == REVIEW_ATTEMPT_REOPEN_METHOD_UNTILPASS) {
    ...
    protected function process_save_grade(&$mform) {
        ...
        $userid = optional_param('userid', 0, PARAM_INT);
        if (!$userid) {
            if (empty($SESSION->mod_review_useridlist[$this->get_useridlist_key($useridlistid)])) {
                ...
                $url = new moodle_url('/mod/review/view.php', array('id' => $this->get_course_module()->id));
                throw new moodle_exception('useridlistnotcached', 'mod_review', $url);
            }
            $useridlist = $SESSION->mod_review_useridlist[$this->get_useridlist_key($useridlistid)];
        } else {
            $useridlist = array($userid);
            $rownum = 0;
        }
     ...
     * @return bool
     */
    public static function can_upgrade_review($type, $version) {
        $review = new review(null, null, null);
        return $review->can_upgrade($type, $version);
    }

    /**
     * This function returns true if it can upgrade an review from the 2.2 module.
     *
     * @param string $type The plugin type
     * @param int $version The plugin version
     * @return bool
    ...
    /**
     * Copy all the files from the old review files area to the new one.
     * This is used by the plugin upgrade code.
     *
     * @param int $oldcontextid The old review context id
     * @param int $oldcomponent The old review component ('review')
     * @param int $oldfilearea The old review filearea ('submissions')
     * @param int $olditemid The old submissionid (can be null e.g. intro)
     * @param int $newcontextid The new review context id
     * @param int $newcomponent The new review component ('review')
     * @param int $newfilearea The new review filearea ('submissions')
    ...
    /**
     * Add a new attempt for each user in the list - but reopen each group review
     * at most 1 time.
     *
    ...
    protected function add_attempt($userid) {
        ...
        if ($this->get_instance()->attemptreopenmethod == REVIEW_ATTEMPT_REOPEN_METHOD_NONE) {
            return false;
        }
        ...
        if ($this->get_instance()->maxattempts != REVIEW_UNLIMITED_ATTEMPTS &&
        ...
        $newsubmission->status = REVIEW_SUBMISSION_STATUS_REOPENED;
    ...
    public function get_user_grades_for_gradebook($userid) {
        ...
        foreach ($this->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                if (('reviewfeedback_' . $plugin->get_type()) == $gradebookpluginname) {
        ...
        $params = array('reviewid1'=>$reviewid,
                        'reviewid2'=>$reviewid,
                        'userid'=>$userid);
        $graderesults = $DB->get_recordset_sql('SELECT
                                                ...
                                                FROM {user} u
                                                LEFT JOIN {review_submission} s
                                                    ON u.id = s.userid and s.review = :reviewid1 AND
                                                    s.latest = 1
                                                JOIN {review_grades} g
                                                    ON u.id = g.userid and g.review = :reviewid2 AND
                                                    g.attemptnumber = s.attemptnumber' .
                                                $where, $params);
    ...
    /**
     * Foreach participant in the course - review them a random id.
     *
     * @param int $reviewid The reviewid to lookup
     */
    public static function allocate_unique_ids($reviewid) {
        global $DB;
        ...
        $users = get_enrolled_users($context, "mod/review:submit", $currentgroup, 'u.id');

        // Shuffle the users.
        shuffle($users);

        foreach ($users as $user) {
            $record = $DB->get_record('review_user_mapping',
                                      array('review'=>$reviewid, 'userid'=>$user->id),
                                     'id');
            if (!$record) {
                $record = new stdClass();
                $record->review = $reviewid;
                $record->userid = $user->id;
                $DB->insert_record('review_user_mapping', $record);
            }
        }
    }

    /**
     * Lookup this user id and return the unique id for this review.
     *
     * @param int $reviewid The review id
     * @param int $userid The userid to lookup
     * @return int The unique id
     */
    public static function get_uniqueid_for_user_static($reviewid, $userid) {
        global $DB;

        // Search for a record.
        $params = array('review'=>$reviewid, 'userid'=>$userid);
        if ($record = $DB->get_record('review_user_mapping', $params, 'id')) {
            return $record->id;
        }
        ...
        self::allocate_unique_ids($reviewid);
        ...
        return $DB->insert_record('review_user_mapping', $record);
    }
    ...
    /**
     * Lookup this unique id and return the user id for this review.
     *
     * @param int $reviewid The id of the review this user mapping is in
     * @param int $uniqueid The uniqueid to lookup
     * @return int The user id or false if they don't exist
     */
    public static function get_user_id_for_uniqueid_static($reviewid, $uniqueid) {
        global $DB;

        // Search for a record.
        if ($record = $DB->get_record('review_user_mapping',
                                      array('review'=>$reviewid, 'id'=>$uniqueid),
                                      'userid',
                                      IGNORE_MISSING)) {
    ...
    public function get_marking_workflow_states_for_current_user() {
        ...
        $states = array();
        if (has_capability('mod/review:grade', $this->context)) {
            $states[REVIEW_MARKING_WORKFLOW_STATE_INMARKING] = get_string('markingworkflowstateinmarking', 'review');
            $states[REVIEW_MARKING_WORKFLOW_STATE_READYFORREVIEW] = get_string('markingworkflowstatereadyforreview', 'review');
        }
        if (has_any_capability(array('mod/review:reviewgrades',
                                     'mod/review:managegrades'), $this->context)) {
            $states[REVIEW_MARKING_WORKFLOW_STATE_INREVIEW] = get_string('markingworkflowstateinreview', 'review');
            $states[REVIEW_MARKING_WORKFLOW_STATE_READYFORRELEASE] = get_string('markingworkflowstatereadyforrelease', 'review');
        }
        if (has_any_capability(array('mod/review:releasegrades',
                                     'mod/review:managegrades'), $this->context)) {
            $states[REVIEW_MARKING_WORKFLOW_STATE_RELEASED] = get_string('markingworkflowstatereleased', 'review');
        }
    ...
    public function get_grading_status($userid) {
        if ($this->get_instance()->markingworkflow) {
            ...
            return REVIEW_MARKING_WORKFLOW_STATE_NOTMARKED;
        } else {
            $attemptnumber = optional_param('attemptnumber', -1, PARAM_INT);
            $grade = $this->get_user_grade($userid, false, $attemptnumber);

            if (!empty($grade) && $grade->grade !== null && $grade->grade >= 0) {
                return REVIEW_GRADING_STATUS_GRADED;
            } else {
                return REVIEW_GRADING_STATUS_NOT_GRADED;
            }
        }
    }

    /**
     * The id used to uniquily identify the cache for this instance of the review object.
     *
     * @return string
     */
    ...
    public function set_module_viewed() {
        ...
        $reviewinstance = $this->get_instance();
        $params = [
            'objectid' => $reviewinstance->id,
            'context' => $this->get_context()
        ];
        if ($this->is_blind_marking()) {
            $params['anonymous'] = 1;
        ...
        $event->add_record_snapshot('review', $reviewinstance);
        $event->trigger();
    }

    /**
     * Checks for any grade notices, and adds notifications. Will display on review main page and grading table.
     *
    ...
    protected function add_grade_notices() {
        if (has_capability('mod/review:grade', $this->get_context()) && get_config('review', 'has_rescaled_null_grades_' . $this->get_instance()->id)) {
            $link = new \moodle_url('/mod/review/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades'));
            \core\notification::warning(get_string('fixrescalednullgrades', 'mod_review', ['link' => $link->out()]));
        }
    }
    ...
    protected function view_fix_rescaled_null_grades() {
        ...
        require_capability('mod/review:grade', $this->get_context());
        ...
        if ($confirm) {
            ...
            unset_config('has_rescaled_null_grades_' . $instance->id, 'review');

            // Display the notice.
            $o .= $this->get_renderer()->notification(get_string('fixrescalednullgradesdone', 'review'), 'notifysuccess');
            $url = new moodle_url(
                '/mod/review/view.php',
                array(
            ...
            $continue = new \moodle_url('/mod/review/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades', 'confirm' => true, 'sesskey' => sesskey()));
            $cancel = new \moodle_url('/mod/review/view.php', array('id' => $this->get_course_module()->id));
            $o .= $OUTPUT->confirm(get_string('fixrescalednullgradesconfirm', 'mod_review'), $continue, $cancel);
        }
     ...
     * attempt on a group review, and whether the gradebook should be updated.
     *
     * @since Moodle 3.4
    ...
    public function get_marking_allocation_filters($export = false) {
        ...
        if ($export) {
            $allocationfilter = get_user_preferences('review_markerfilter', '');
            $result = [];
    ...
    public function get_marking_workflow_filters($export = false) {
        ...
        if ($markingworkflow) {
            $notmarked = get_string('markingworkflowstatenotmarked', 'review');
            $markingworkflowoptions[''] = get_string('filternone', 'review');
            $markingworkflowoptions[REVIEW_MARKING_WORKFLOW_STATE_NOTMARKED] = $notmarked;
    ...
    public function get_filters() {
        $filterkeys = [
            REVIEW_FILTER_SUBMITTED,
            REVIEW_FILTER_NOT_SUBMITTED,
            REVIEW_FILTER_REQUIRE_GRADING,
            REVIEW_FILTER_GRANTED_EXTENSION
        ];

        $current = get_user_preferences('review_filter', '');

        $filters = [];
        // First is always "no filter" option.
        array_push($filters, [
            'key' => 'none',
            'name' => get_string('filternone', 'review'),
            'active' => ($current == '')
        ]);
        ...
        foreach ($filterkeys as $key) {
            array_push($filters, [
                'key' => $key,
                'name' => get_string('filter' . $key, 'review'),
                'active' => ($current == $key)
            ]);
        }
        return $filters;
...
/**
 * Portfolio caller class for mod_review.
 *
 * @package   mod_review
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_portfolio_caller extends portfolio_module_caller_base {
    ...
    /** @var int callback arg - the cmid of the review we export */
    protected $cmid;
    ...
    public function __construct($callbackargs) {
        ...
        $this->cm = get_coursemodule_from_id('review', $this->cmid, 0, false, MUST_EXIST);
    }

    /**
     * Load data needed for the portfolio export.
     *
     * If the review type implements portfolio_load_data(), the processing is delegated
    ...
    public function load_data() {
        ...
        if (empty($this->fileid)) {
            if (empty($this->sid) || empty($this->area)) {
                throw new portfolio_caller_exception('invalidfileandsubmissionid', 'mod_review');
            }

            $submission = $DB->get_record('review_submission', array('id' => $this->sid));
        } else {
            $submissionid = $DB->get_field('files', 'itemid', array('id' => $this->fileid, 'contextid' => $context->id));
            if ($submissionid) {
                $submission = $DB->get_record('review_submission', array('id' => $submissionid));
            }
        }
    ...
    public function prepare_package() {
        ...
        if ($this->plugin && $this->editor) {
            ...
            if (in_array($exporterclass, array(PORTFOLIO_FORMAT_PLAINHTML, PORTFOLIO_FORMAT_RICHHTML))) {
                ...
                return $this->exporter->write_new_file($html, 'review.html', !empty($files));
        ...
        if ($this->exporter->get('formatclass') == PORTFOLIO_FORMAT_LEAP2A) {
            ...
            if (count($files) > 1) {
                $baseid = 'review' . $this->cmid . $this->area;
                $context = context_module::instance($this->cmid);
     ...
     *
     * @return review_submission_plugin
     */
    protected function get_submission_plugin() {
        ...
        $review = new review($context, null, null);
        return $review->get_submission_plugin_by_type($this->plugin);
    }
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cmid);
        return has_capability('mod/review:exportownsubmission', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'review');
    }
 ...
 * @return void
 */
function review_process_group_deleted_in_course($courseid, $groupid = null) {
    global $DB;
    ...
    if ($groupid) {
        ...
        $sql = "SELECT o.id, o.reviewid
                  FROM {review_overrides} o
                  JOIN {review} review ON review.id = o.reviewid
                 WHERE review.course = :courseid
                   AND o.groupid = :groupid";
    } else {
        // No groupid, we update all orphaned group overrides for all review in course.
        $sql = "SELECT o.id, o.reviewid
                  FROM {review_overrides} o
                  JOIN {review} review ON review.id = o.reviewid
    ...
    $DB->delete_records_list('review_overrides', 'id', array_keys($records));
}
 ...
 * @param int $reviewid of the review
 * @return bool success of operation
 */
function move_group_override_($id, $move, $reviewid) {
    global $DB;

    // Get the override object.
    if (!$override = $DB->get_record('review_overrides', array('id' => $id), 'id, sortorder')) {
        return false;
    }
    // Count the number of group overrides.
    $overridecountgroup = $DB->count_records('review_overrides', array('userid' => null, 'reviewid' => $reviewid));
    ...
    $params = array('sortorder' => $neworder, 'reviewid' => $reviewid);
    if ($swapoverride = $DB->get_record('review_overrides', $params, 'id, sortorder')) {
        ...
        $DB->update_record('review_overrides', $override);
        $DB->update_record('review_overrides', $swapoverride);
    }

    reorder_group_overrides_($reviewid);
    return true;
}
 ...
 *
 * @param int $reviewid of the assigment
 */
function reorder_group_overrides_($reviewid) {
    global $DB;

    $i = 1;
    if ($overrides = $DB->get_records('review_overrides', array('userid' => null, 'reviewid' => $reviewid), 'sortorder ASC')) {
        foreach ($overrides as $override) {
            $f = new stdClass();
            $f->id = $override->id;
            $f->sortorder = $i++;
            $DB->update_record('review_overrides', $f);

            // Update priorities of group overrides.
            $params = [
                'modulename' => 'review',
                'instance' => $override->reviewid,
