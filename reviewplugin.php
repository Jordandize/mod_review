...
/**
 * This file contains the functions for review_plugin abstract class
 *
 *
 * @package   mod_review
...
/**
 * Abstract class for review_plugin (submission/feedback).
 *
 * @package   mod_review
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class review_plugin {

    /** @var review $review the review record that contains the global
     *              settings for this review instance
     */
    protected $review;
    /** @var string $type review plugin type */
    private $type = '';
     ...
     * @param string $type
     */
    public final function __construct(review $review, $type) {
        $this->review = $review;
        $this->type = $type;
    }
     ...
     * form for display in the settings page for the review.
    ...
    /**
     * The review subtype is responsible for saving it's own settings as the database table for the
     ...
     *
     * @return string - either 'reviewsubmission' or 'feedback'
     */
     ...
     *
     * @param stdClass $submissionorgrade - review_submission or review_grade.
    ...
    /**
     * Get any additional fields for the submission/grading form for this review.
     *
    ...
    public final function has_admin_settings() {
        ...
        $pluginroot = $CFG->dirroot . '/mod/review/' . substr($this->get_subtype(), strlen('review')) . '/' . $this->get_type();
    ...
    public final function set_config($name, $value) {
        ...
        $dbparams = array('review'=>$this->review->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                          'plugin'=>$this->get_type(),
                          'name'=>$name);
        $current = $DB->get_record('review_plugin_config', $dbparams, '*', IGNORE_MISSING);

        if ($current) {
            $current->value = $value;
            return $DB->update_record('review_plugin_config', $current);
        } else {
            $setting = new stdClass();
            $setting->review = $this->review->get_instance()->id;
            $setting->subtype = $this->get_subtype();
            $setting->plugin = $this->get_type();
            $setting->name = $name;
            $setting->value = $value;

            return $DB->insert_record('review_plugin_config', $setting) > 0;
        }
    }
    ...
    public final function get_config($setting = null) {
        ...
        if ($setting) {
            ...
            if ($review) {
                $dbparams = array('review'=>$review->id,
                                  'subtype'=>$this->get_subtype(),
                                  'plugin'=>$this->get_type(),
                                  'name'=>$setting);
                $result = $DB->get_record('review_plugin_config', $dbparams, '*', IGNORE_MISSING);
                if ($result) {
                    return $result->value;
                }
            }
        ...
        $dbparams = array('review'=>$this->review->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                           'plugin'=>$this->get_type());
        $results = $DB->get_records('review_plugin_config', $dbparams);
     ...
     * and version.
     *
     * @param string $type The old review subtype
     * @param int $version The old review version
    ...
    /**
     * Upgrade the settings from the old review to the new one
     *
     * @param context $oldcontext The context for the old review module
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldreview, & $log) {
        ...
        $log .= ' ' . get_string('upgradenotimplemented', 'mod_review', $params);
        return false;
    }
     ...
     * @param stdClass $submissionorgrade review_submission or review_grade The new submission or grade
     * @param string $log Record upgrade messages in the log
    ...
    public function upgrade(context $oldcontext,
                            ...
                            & $log) {
        $params = array('type'=>$this->type, 'subtype'=>$this->get_subtype());
        $log = $log . ' ' . get_string('upgradenotimplemented', 'mod_review', $params);
        return false;
    }
    ...
    /**
     * The review has been deleted - remove the plugin specific data
     *
     * @return bool
     */
    ...
    /**
     * Is this review plugin empty? (ie no submission or feedback)
     * @param stdClass $submissionorgrade review_submission or review_grade
     * @return bool
     */
    ...
    public function get_file_info($browser, $filearea, $itemid, $filepath, $filename) {
        ...
        if ($this->get_subtype() == 'reviewsubmission') {
            if ($itemid) {
                $record = $DB->get_record('review_submission', array('id' => $itemid), 'userid,groupid', IGNORE_MISSING);
                if (!$record) {
                    return null;
                }
                if (!empty($record->userid)) {
                    if (!$review->can_view_submission($record->userid)) {
                        return null;
                    }

                    // We only report write access for teachers.
                    $writeaccess = $review->can_grade() && $review->can_edit_submission($record->userid);
                } else {
                    // Must be a team submission with a group.
                    if (!$review->can_view_group_submission($record->groupid)) {
                        return null;
                    }
                    // We only report write access for teachers.
                    $writeaccess = $review->can_grade() && $review->can_edit_group_submission($record->groupid);
                }
            }
        } else {
    ...
    /**
     * This allows a plugin to render a page in the context of the review
     *
     * If the plugin creates a link to the review view.php page with
     * The following required parameters:
     *      id=coursemoduleid
     *      plugin=type
     *      pluginsubtype=reviewfeedback|reviewsubmission
     ...
     * right below the activity's "intro" section on the main review page.
     *
     * @return string
     */
     ...
     * enabled/disabled per review instance.
     *
     * @return bool
     */
