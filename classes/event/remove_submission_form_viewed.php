...
/**
 * The mod_review remove submission form viewed event.
 *
 * @package    mod_review
...
/**
 * The mod_review remove submission form viewed event class.
 *
 * @package    mod_review
 * @copyright  2019 Damyon Wiese
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class remove_submission_form_viewed extends base {
    ...
    public static function get_name() {
        return get_string('eventremovesubmissionformviewed', 'mod_review');
    }
    ...
    public function get_description() {
        ...
        return "The user with id '$this->userid' viewed their remove submission form for the review with course module id " .
    ...
    protected function get_legacy_logdata() {
        if ($this->relateduserid == $this->userid) {
            $title = get_string('removesubmission', 'review');
        } else {
            $user = $this->get_record_snapshot('user', $this->relateduserid);
            $title = get_string('removesubmissionforstudent', 'review', array('id' => $user->id, 'fullname' => fullname($user)));
        }
        $this->set_legacy_logdata('view remove submission review form', $title);
