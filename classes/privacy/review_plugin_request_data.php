...
/**
 * This file contains the mod_review review_plugin_request_data class
 *
 * For review plugin privacy data to fulfill requests.
 *
 * @package mod_review
...
/**
 * An object for fulfilling an review plugin data request.
 *
 * @copyright  2018 Adrian Greeve <adrian@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_plugin_request_data {
    ...
    /** @var review The review object */
    protected $review;

    /**
     * Object creator for review plugin request data.
     *
     ...
     */
    public function __construct(\context $context, \review $review, \stdClass $pluginobject = null, array $subcontext = [],
            \stdClass $user = null) {
     ...
     * Getter for this attribute.
     *
     * @return object The review plugin object
     */
    ...
    /**
     * A method to conveniently fetch the review id.
     *
     * @return int The review id.
     */
    public function get_reviewid() {
        return $this->review->get_instance()->id;
    }
    ...
    public function populate_submissions_and_grades() {
        ...
        $params['review'] = $this->get_review()->get_instance()->id;
        $this->submissions = $DB->get_records_select('review_submission', "review = :review AND userid $sql", $params);
        $this->grades = $DB->get_records_select('review_grades', "review = :review AND userid $sql", $params);
    }
}
