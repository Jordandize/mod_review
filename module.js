...
M.mod_review.init_tree = function(Y, expand_all, htmlid) {
...
M.mod_review.init_grading_table = function(Y) {
        ...
        if (batchform) {
            batchform.on('submit', function(e) {
                ...
                if (selectedusers.length == 0) {
                    alert(M.util.get_string('nousersselected', 'review'));
                    e.preventDefault();
                } else {
                    ...
                    if (action.indexOf(prefix) == 0) {
                        ...
                        confirmmessage = M.util.get_string('batchoperationconfirm' + action, 'reviewfeedback_' + plugin);
                    } else {
                        confirmmessage = M.util.get_string('batchoperationconfirm' + operation.get('value'), 'review');
                    }
...
M.mod_review.init_grading_options = function(Y) {
    Y.use('node', function(Y) {
...
M.mod_review.init_plugin_summary = function(Y, subtype, type, submissionid) {
