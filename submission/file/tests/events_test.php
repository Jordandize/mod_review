...
class reviewsubmission_file_events_testcase extends advanced_testcase {
    ...
    public function test_assessable_uploaded() {
        ...
        $dummy = (object) array(
            ...
            'itemid' => $submission->id,
            'filepath' => '/',
            'filename' => 'myreviewmnent.png'
        );
        $fi2 = $fs->create_file_from_string($dummy, 'Content of ' . $dummy->filename);
        $files = $fs->get_area_files($context->id, 'reviewsubmission_file', REVIEWSUBMISSION_FILE_FILEAREA,
        ...
        $event = reset($events);
        $this->assertInstanceOf('\reviewsubmission_file\event\assessable_uploaded', $event);
    ...
    public function test_submission_created() {
        ...
        $event = $events[1];
        $this->assertInstanceOf('\reviewsubmission_file\event\submission_created', $event);
        $this->assertEquals($context->id, $event->contextid);
    ...
    public function test_submission_updated() {
        ...
        // We want to test the last event fired.
        $event = $events[3];
        $this->assertInstanceOf('\reviewsubmission_file\event\submission_updated', $event);
        $this->assertEquals($context->id, $event->contextid);
        $this->assertEquals($course->id, $event->courseid);
        $this->assertEquals($submission->id, $event->other['submissionid']);
        $this->assertEquals($submission->attemptnumber, $event->other['submissionattempt']);
