...
namespace reviewsubmission_comments\privacy;
...
class provider implements metadataprovider,
    ...
    public static function get_context_for_userid_within_submission(int $userid, contextlist $contextlist) {
        $sql = "SELECT contextid
                 ...
                 WHERE component = :component
                       ...
                       AND userid = :userid";
        $params = ['userid' => $userid, 'component' => 'reviewsubmission_comments', 'commentarea' => 'submission_comments'];
        $contextlist->add_from_sql($sql, $params);
    }

    /**
     * Due to the fact that we can't rely on the queries in the mod_review provider we have to add some additional sql.
     *
    ...
    public static function get_student_user_ids(\mod_review\privacy\useridlist $useridlist) {
        $params = ['reviewid' => $useridlist->get_reviewid(), 'commentuserid' => $useridlist->get_teacherid(),
                'commentuserid2' => $useridlist->get_teacherid()];
        $sql = "SELECT DISTINCT c.userid AS id
                  FROM {comments} c
                  JOIN (SELECT c.itemid
                          FROM {comments} c
                          JOIN {review_submission} s ON s.id = c.itemid AND s.review = :reviewid
    ...
    public static function get_userids_from_context(\core_privacy\local\request\userlist $userlist) {
        ...
        comments_provider::get_users_in_context_from_sql($userlist, 'c', 'reviewsubmission_comments', 'submission_comments',
                $context->id);
    }
     ...
     *
     * @param  review_plugin_request_data $requestdata Data to fulfill the deletion request.
     */
    public static function delete_submission_for_context(review_plugin_request_data $requestdata) {
        comments_provider::delete_comments_for_all_users($requestdata->get_context(), 'reviewsubmission_comments',
                'submission_comments');
    }
     ...
     *
     * @param  review_plugin_request_data $exportdata Details about the user and context to focus the deletion.
     */
    public static function delete_submission_for_userid(review_plugin_request_data $exportdata) {
        // Create an approved context list to delete the comments.
        $contextlist = new \core_privacy\local\request\approved_contextlist($exportdata->get_user(), 'reviewsubmission_comments',
            [$exportdata->get_context()->id]);
        comments_provider::delete_comments_for_user($contextlist, 'reviewsubmission_comments', 'submission_comments');
    }
    ...
    public static function delete_submissions(review_plugin_request_data $deletedata) {
        $userlist = new \core_privacy\local\request\approved_userlist($deletedata->get_context(), 'reviewsubmission_comments',
                $deletedata->get_userids());
        comments_provider::delete_comments_for_users($userlist, 'reviewsubmission_comments', 'submission_comments');
    }
