 ...
 * @return bool
 */
function reviewsubmission_comments_comment_validate(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if (!$submission = $DB->get_record('review_submission', array('id'=>$options->itemid))) {
    ...
    if ($review->get_instance()->id != $submission->review) {
 ...
 * @return array
 */
function reviewsubmission_comments_comment_permissions(stdClass $options) {
    global $USER, $CFG, $DB;
    ...
    if ($review->get_instance()->teamsubmission &&
        !$review->can_view_group_submission($submission->groupid)) {
        return array('post' => false, 'view' => false);
    }

    if (!$review->get_instance()->teamsubmission &&
        !$review->can_view_submission($submission->userid)) {
        return array('post' => false, 'view' => false);
    }
 ...
 */
function reviewsubmission_comments_comment_display($comments, $options) {
    global $CFG, $DB, $USER;
    ...
    if ($review->is_blind_marking() && !empty($comments)) {
        ...
        $userinteam = false;
        if ($review->get_instance()->teamsubmission && has_capability('mod/review:submit', $context)) {
            $review->set_course(get_course($course));
            $userinteam = $review->can_edit_group_submission($submission->groupid);
        }

        foreach ($comments as $comment) {

            if (has_capability('mod/review:viewblinddetails', $context) && $USER->id != $comment->userid) {
                $anonid = $review->get_uniqueid_for_user($comment->userid);
                ...
                $comment->fullname = get_string('blindmarkingviewfullname', 'reviewsubmission_comments', $a);
                ...
                if (empty($usermappings[$comment->userid])) {
                    ...
                    $commenter->firstname = get_string('blindmarkingname', 'reviewsubmission_comments', $anonid);
                    $commenter->lastname = '';
                    ...
                    $usermappings[$comment->userid]->avatar = $review->get_renderer()->user_picture($commenter,
 ...
 */
function reviewsubmission_comments_comment_add(stdClass $comment, stdClass $param) {
