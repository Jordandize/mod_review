...
/**
 * Unit tests for reviewsubmission_comments.
 *
...
/**
 * Unit tests for mod/review/submission/comments/classes/privacy/
 *
 * @copyright  2018 Adrian Greeve <adrian@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class reviewsubmission_comments_privacy_testcase extends \mod_review\tests\mod_review_privacy_testcase {
     ...
     */
    protected function create_comment_submission($review, $student, $submissiontext) {
        ...
        $options->course = $review->get_course();
    ...
    public function test_get_metadata() {
        $collection = new \core_privacy\local\metadata\collection('reviewsubmission_comments');
        $collection = \reviewsubmission_comments\privacy\provider::get_metadata($collection);
        $this->assertNotEmpty($collection);
    }

    /**
     * Test returning the context for a user who has made a comment in an review.
     */
    public function test_get_context_for_userid_within_submission() {
        ...
        list($plugin, $submission, $comment) = $this->create_comment_submission($review, $user1, $studentcomment);
        $teachercomment = 'From the teacher';
        $this->setUser($user2);
        $comment->add($teachercomment);

        $contextlist = new \core_privacy\local\request\contextlist();
        \reviewsubmission_comments\privacy\provider::get_context_for_userid_within_submission($user2->id, $contextlist);
    ...
    public function test_get_student_user_ids() {
        ...
        $useridlist = new mod_review\privacy\useridlist($user2->id, $review->get_instance()->id);
        \reviewsubmission_comments\privacy\provider::get_student_user_ids($useridlist);
    ...
    public function test_get_userids_from_context() {
        ...
        $reviewcontext1 = $review1->get_context();
        $reviewcontext2 = $review2->get_context();

        $user1comment = 'Comment from user 1';
        list($plugin, $submission, $comment) = $this->create_comment_submission($review1, $user1, $user1comment);
        ...
        list($plugin, $submission, $comment) = $this->create_comment_submission($review2, $user4, $user4comment);
        $user3secondcomment = 'Comment on user 4 post.';
        $this->setUser($user3);
        $comment->add($user3comment);

        $userlist = new \core_privacy\local\request\userlist($reviewcontext1, 'reviewsubmission_comments');
        \reviewsubmission_comments\privacy\provider::get_userids_from_context($userlist);
    ...
    public function test_export_submission_user_data() {
        ...
        $exportdata = new \mod_review\privacy\review_plugin_request_data($context, $review, $submission);
        \reviewsubmission_comments\privacy\provider::export_submission_user_data($exportdata);
    ...
    public function test_delete_submission_for_context() {
        ...
        list($plugin2, $submission2, $comment2) = $this->create_comment_submission($review, $user2, $studentcomment);
        ...
        \reviewsubmission_comments\privacy\provider::delete_submission_for_context($requestdata);
    ...
    public function test_delete_submission_for_userid() {
        ...
        $requestdata = new \mod_review\privacy\review_plugin_request_data($context, $review, null, [], $user1);
        \reviewsubmission_comments\privacy\provider::delete_submission_for_userid($requestdata);
    ...
    public function test_delete_submissions() {
        ...
        $results = $DB->get_records('comments', ['contextid' => $reviewcontext1->id]);
        $this->assertCount(3, $results);

        $deletedata = new \mod_review\privacy\review_plugin_request_data($reviewcontext1, $review1);
        $deletedata->set_userids([$user1->id, $user3->id]);
        \reviewsubmission_comments\privacy\provider::delete_submissions($deletedata);
