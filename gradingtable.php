...
/**
 * Extends table_sql to provide a table of review submissions
 *
 * @package   mod_review
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class review_grading_table extends table_sql implements renderable {
    /** @var review $review */
    private $review = null;
    /** @var int $perpage */
    private $perpage = 10;
    ...
    /**
     * overridden constructor keeps a reference to the review class that is displaying this table
     *
    ...
    public function __construct(review $review,
        ...
        parent::__construct('mod_review_grading');
        $this->is_persistent(true);
        $this->review = $review;

        // Check permissions up front.
        $this->hasgrantextension = has_capability('mod/review:grantextension',
                                                  $this->review->get_context());
        $this->hasgrade = $this->review->can_grade();

        // Check if we have the elevated view capablities to see the blind details.
        $this->hasviewblind = has_capability('mod/review:viewblinddetails',
                $this->review->get_context());
        ...
        $this->output = $PAGE->get_renderer('mod_review');

        $urlparams = array('action' => 'grading', 'id' => $review->get_course_module()->id);
        $url = new moodle_url($CFG->wwwroot . '/mod/review/view.php', $urlparams);
        ...
        $users = array_keys( $review->list_participants($currentgroup, true));
        if (count($users) == 0) {
        ...
        $params = array();
        $params['reviewid1'] = (int)$this->review->get_instance()->id;
        $params['reviewid2'] = (int)$this->review->get_instance()->id;
        $params['reviewid3'] = (int)$this->review->get_instance()->id;
        $params['newstatus'] = REVIEW_SUBMISSION_STATUS_NEW;

        $extrauserfields = get_extra_user_fields($this->review->get_context());
        ...
        // For group reviews, there can be a grade with no submission.
        $from .= ' LEFT JOIN {review_grades} g
                            ON g.review = :reviewid2
                           AND u.id = g.userid
                           AND (g.attemptnumber = s.attemptnumber OR s.attemptnumber IS NULL) ';

        $from .= 'LEFT JOIN {review_user_flags} uf
                         ON u.id = uf.userid
                        AND uf.review = :reviewid3 ';

        $hasoverrides = $this->review->has_overrides();

        if ($hasoverrides) {
            $params['reviewid5'] = (int)$this->review->get_instance()->id;
            $params['reviewid6'] = (int)$this->review->get_instance()->id;
            $params['reviewid7'] = (int)$this->review->get_instance()->id;
            $params['reviewid8'] = (int)$this->review->get_instance()->id;
            $params['reviewid9'] = (int)$this->review->get_instance()->id;
            ...
            $from .= ' LEFT JOIN (
               SELECT merged.userid, min(merged.priority) priority FROM (
                  ...
                  ( SELECT uo.userid, 0 AS priority
                      FROM {review_overrides} uo
                     WHERE uo.reviewid = :reviewid5
                  )
                  UNION
                  ( SELECT gm.userid, go.sortorder AS priority
                      FROM {review_overrides} go
                      JOIN {groups} g ON g.id = go.groupid
                      JOIN {groups_members} gm ON gm.groupid = g.id
                     WHERE go.reviewid = :reviewid6
                  )
                ) merged
            ...
            JOIN (
              (SELECT 9999999 AS priority,
                      ...
                      a.cutoffdate
                 FROM {user} u
                 JOIN {review} a ON a.id = :reviewid7
              )
              UNION
              ...
              (SELECT go.sortorder AS priority,
                 ...
                 JOIN {groups} g ON g.id = go.groupid
                 JOIN {groups_members} gm ON gm.groupid = g.id
                WHERE go.reviewid = :reviewid9
              )

            ) effective ON effective.priority = priority.priority AND effective.userid = priority.userid ';
        }

        if (!empty($this->review->get_instance()->blindmarking)) {
            $from .= 'LEFT JOIN {review_user_mapping} um
                             ON u.id = um.userid
                            AND um.review = :reviewidblind ';
            $params['reviewidblind'] = (int)$this->review->get_instance()->id;
        ...
        if ($this->review->is_any_submission_plugin_enabled()) {
            if ($filter == REVIEW_FILTER_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted) ';
                $params['submitted'] = REVIEW_SUBMISSION_STATUS_SUBMITTED;

            } else if ($filter == REVIEW_FILTER_NOT_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NULL OR s.status <> :submitted) ';
                $params['submitted'] = REVIEW_SUBMISSION_STATUS_SUBMITTED;
            } else if ($filter == REVIEW_FILTER_REQUIRE_GRADING) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted AND
                                 (s.timemodified >= g.timemodified OR g.timemodified IS NULL OR g.grade IS NULL';

                // Review grade is set to the negative grade scale id when scales are used.
            ...
            } else if ($filter == REVIEW_FILTER_GRANTED_EXTENSION) {
                $where .= ' AND uf.extensionduedate > 0 ';

            } else if (strpos($filter, REVIEW_FILTER_SINGLE_USER) === 0) {
        ...
        if ($this->review->get_instance()->markingworkflow &&
            $this->review->get_instance()->markingallocation) {
            if (has_capability('mod/review:manageallocations', $this->review->get_context())) {
                // Check to see if marker filter is set.
                $markerfilter = (int)get_user_preferences('review_markerfilter', '');
                if (!empty($markerfilter)) {
                    if ($markerfilter == REVIEW_MARKER_FILTER_NO_MARKER) {
        ...
        if ($this->review->get_instance()->markingworkflow) {
            $workflowstates = $this->review->get_marking_workflow_states_for_current_user();
            if (!empty($workflowstates)) {
                $workflowfilter = get_user_preferences('review_workflowfilter', '');
                if ($workflowfilter == REVIEW_MARKING_WORKFLOW_STATE_NOTMARKED) {
                    $where .= ' AND (uf.workflowstate = :workflowstate OR uf.workflowstate IS NULL OR '.
                        $DB->sql_isempty('review_user_flags', 'workflowstate', true, true).')';
        ...
        // User picture.
        if ($this->hasviewblind || !$this->review->is_blind_marking()) {
            if (!$this->is_downloading()) {
                $columns[] = 'picture';
                $headers[] = get_string('pictureofuser');
            } else {
                $columns[] = 'recordid';
                $headers[] = get_string('recordid', 'review');
            }
        ...
        // Submission status.
        $columns[] = 'status';
        $headers[] = get_string('status', 'review');

        if ($hasoverrides) {
            // Allowsubmissionsfromdate.
            $columns[] = 'allowsubmissionsfromdate';
            $headers[] = get_string('allowsubmissionsfromdate', 'review');

            // Duedate.
            $columns[] = 'duedate';
            $headers[] = get_string('duedate', 'review');

            // Cutoffdate.
            $columns[] = 'cutoffdate';
            $headers[] = get_string('cutoffdate', 'review');
        }

        // Team submission columns.
        if ($review->get_instance()->teamsubmission) {
            $columns[] = 'team';
            $headers[] = get_string('submissionteam', 'review');
        }
        // Allocated marker.
        if ($this->review->get_instance()->markingworkflow &&
            $this->review->get_instance()->markingallocation &&
            has_capability('mod/review:manageallocations', $this->review->get_context())) {
            // Add a column for the allocated marker.
            $columns[] = 'allocatedmarker';
            $headers[] = get_string('marker', 'review');
        }
        // Grade.
        $columns[] = 'grade';
        $headers[] = get_string('grade');
        if ($this->is_downloading()) {
            $gradetype = $this->review->get_instance()->grade;
            if ($gradetype > 0) {
                $columns[] = 'grademax';
                $headers[] = get_string('maxgrade', 'review');
            } else if ($gradetype < 0) {
                // This is a custom scale.
                $columns[] = 'scale';
                $headers[] = get_string('scale', 'review');
            }

            if ($this->review->get_instance()->markingworkflow) {
                // Add a column for the marking workflow state.
                $columns[] = 'workflowstate';
                $headers[] = get_string('markingworkflowstate', 'review');
            }
            // Add a column to show if this grade can be changed.
            $columns[] = 'gradecanbechanged';
            $headers[] = get_string('gradecanbechanged', 'review');
        }
        if (!$this->is_downloading() && $this->hasgrade) {
        ...
        // Submission plugins.
        if ($review->is_any_submission_plugin_enabled()) {
            $columns[] = 'timesubmitted';
            $headers[] = get_string('lastmodifiedsubmission', 'review');

            foreach ($this->review->get_submission_plugins() as $plugin) {
                if ($this->is_downloading()) {
                    if ($plugin->is_visible() && $plugin->is_enabled()) {
        ...
        // Time marked.
        $columns[] = 'timemarked';
        $headers[] = get_string('lastmodifiedgrade', 'review');

        // Feedback plugins.
        foreach ($this->review->get_feedback_plugins() as $plugin) {
            if ($this->is_downloading()) {
                if ($plugin->is_visible() && $plugin->is_enabled()) {
                    foreach ($plugin->get_editor_fields() as $field => $description) {
                        $index = 'plugin' . count($this->plugincache);
        ...
        $this->gradinginfo = grade_get_grades($this->review->get_course()->id,
                                              'mod',
                                              'review',
                                              $this->review->get_instance()->id,
                                              $users);
    ...
    /**
     * Add a column with an ID that uniquely identifies this user in this review.
     *
     * @param stdClass $row
     * @return string
     */
    public function col_recordid(stdClass $row) {
        if (empty($row->recordid)) {
            $row->recordid = $this->review->get_uniqueid_for_user($row->userid);
        }
        return get_string('hiddenuser', 'review') . $row->recordid;
    }
    ...
    public function col_workflowstatus(stdClass $row) {
        ...
        $gradingdisabled = $this->review->grading_disabled($row->id);
        // The function in the review keeps a static cache of this list of states.
        $workflowstates = $this->review->get_marking_workflow_states_for_current_user();
        ...
        if ($this->quickgrading && !$gradingdisabled) {
            ...
            $o .= $this->output->container(get_string('markingworkflowstate' . $workflowstate, 'review'), $workflowstate);
        }
        return $o;
    }
    ...
    public function col_workflowstate($row) {
        ...
        if (empty($state)) {
            $state = REVIEW_MARKING_WORKFLOW_STATE_NOTMARKED;
        }

        return get_string('markingworkflowstate' . $state, 'review');
    }
    ...
    public function col_allocatedmarker(stdClass $row) {
        ...
        if ($markers === null) {
            ...
            $markers = get_enrolled_users($this->review->get_context(), 'mod/review:grade', 0, 'u.*', $sort);
            $markerlist[0] = get_string('choosemarker', 'review');
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->review->get_context());
        ...
        if ($this->is_downloading()) {
            if (isset($markers[$row->allocatedmarker])) {
                return fullname($markers[$row->allocatedmarker],
                        has_capability('moodle/site:viewfullnames', $this->review->get_context()));
            } else {
                return '';
            }
        }

        if ($this->quickgrading && has_capability('mod/review:manageallocations', $this->review->get_context()) &&
            (empty($row->workflowstate) ||
             $row->workflowstate == REVIEW_MARKING_WORKFLOW_STATE_INMARKING ||
             $row->workflowstate == REVIEW_MARKING_WORKFLOW_STATE_NOTMARKED)) {
    ...
    public function col_scale($row) {
        ...
        if (empty($this->scale)) {
            $dbparams = array('id' => -($this->review->get_instance()->grade));
    ...
    public function display_grade($grade, $editable, $userid, $modified) {
        if ($this->is_downloading()) {
            if ($this->review->get_instance()->grade >= 0) {
                ...
                $gradeitem = $this->review->get_grade_item();
                return format_float($grade, $gradeitem->get_decimals());
            } else {
                // This is a custom scale.
                $scale = $this->review->display_grade($grade, false);
                if ($scale == '-') {
                    $scale = '';
                }
                return $scale;
        ...
        return $this->review->display_grade($grade, $editable, $userid, $modified);
    }
    ...
    public function col_team(stdClass $row) {
        ...
        if ($group) {
            return $group->name;
        } else if ($this->review->get_instance()->preventsubmissionnotingroup) {
            $usergroups = $this->review->get_all_groups($row->id);
            if (count($usergroups) > 1) {
                return get_string('multipleteamsgrader', 'review');
            } else {
                return get_string('noteamgrader', 'review');
            }
        }
        return get_string('defaultteam', 'review');
    }
    ...
    protected function get_group_and_submission($userid, &$group, &$submission, $attemptnumber) {
        ...
        if (isset($this->submissiongroups[$userid])) {
            ...
            $group = $this->review->get_submission_group($userid, false);
        ...
        if (isset($this->groupsubmissions[$groupid . ':' . $attemptnumber])) {
            ...
            $submission = $this->review->get_group_submission($userid, $groupid, false, $attemptnumber);
    ...
    public function col_fullname($row) {
        if (!$this->is_downloading()) {
            ...
            $fullname = $this->output->action_link($link, $this->review->fullname($row));
        } else {
            $fullname = $this->review->fullname($row);
        }

        if (!$this->review->is_active_user($row->id)) {
    ...
    public function col_select(stdClass $row) {
        ...
        $selectcol .= get_string('selectuser', 'review', $this->review->fullname($row));
        $selectcol .= '</label>';
    ...
    /**
     * Return a users grades from the listing of all grade data for this review.
     *
     * @param int $userid
    ...
    public function col_grademax(stdClass $row) {
        if ($this->review->get_instance()->grade > 0) {
            $gradeitem = $this->review->get_grade_item();
            return format_float($this->review->get_instance()->grade, $gradeitem->get_decimals());
        } else {
            return '';
        }
    }
    ...
    public function col_grade(stdClass $row) {
        ...
        if (!$this->is_downloading() && $this->hasgrade) {
            $urlparams = array('id' => $this->review->get_course_module()->id,
                               'rownum' => 0,
                               'action' => 'grader');
    ...
    public function col_timesubmitted(stdClass $row) {
        ...
        $submission = false;
        $this->get_group_and_submission($row->id, $group, $submission, -1);
        if ($submission && $submission->timemodified && $submission->status != REVIEW_SUBMISSION_STATUS_NEW) {
            $o = userdate($submission->timemodified);
        } else if ($row->timesubmitted && $row->status != REVIEW_SUBMISSION_STATUS_NEW) {
    ...
    public function col_status(stdClass $row) {
        ...
        $instance = $this->review->get_instance();
        ...
        if ($this->review->is_any_submission_plugin_enabled()) {
            ...
            $o .= $this->output->container(get_string('submissionstatus_' . $displaystatus, 'review'),
                                           array('class' => 'submissionstatus' .$displaystatus));
            if ($due && $timesubmitted > $due && $status != REVIEW_SUBMISSION_STATUS_NEW) {
            ...
            if ($row->locked) {
                $lockedstr = get_string('submissionslockedshort', 'review');
            ...
            if (!$instance->markingworkflow) {
                if ($row->grade !== null && $row->grade >= 0) {
                    if ($row->timemarked < $row->timesubmitted) {
                        $o .= $this->output->container(get_string('gradedfollowupsubmit', 'review'), 'gradingreminder');
                    } else {
                        $o .= $this->output->container(get_string('graded', 'review'), 'submissiongraded');
                    }
                } else if (!$timesubmitted || $status == REVIEW_SUBMISSION_STATUS_NEW) {
                    $now = time();
                    if ($due && ($now > $due)) {
                        $overduestr = get_string('overdue', 'review', format_time($now - $due));
        ...
        if ($row->extensionduedate) {
            ...
            $extensionstr = get_string('userextensiondate', 'review', $userdate);
    ...
    public function col_userid(stdClass $row) {
        ...
        if (!$row->grade) {
            ...
            $description = get_string('updategrade', 'review');
        }
        ...
        $submission = $row;
        $flags = $row;
        if ($this->review->get_instance()->teamsubmission) {
        ...
        $submissionsopen = $this->review->submissions_open($row->id,
                                                               true,
                                                               $submission,
                                                               $flags,
                                                               $this->gradinginfo);
        $caneditsubmission = $this->review->can_edit_submission($row->id, $USER->id);

        // Hide for offline reviews.
        if ($this->review->is_any_submission_plugin_enabled()) {
            if (!$row->status ||
                    $row->status == REVIEW_SUBMISSION_STATUS_DRAFT ||
                    !$this->review->get_instance()->submissiondrafts) {
                ...
                if (!$row->locked) {
                    ...
                    $description = get_string('preventsubmissionsshort', 'review');
                    ...
                    $urlparams = array('id' => $this->review->get_course_module()->id,
                                       ...
                                       'page' => $this->currpage);
                    $url = new moodle_url('/mod/review/view.php', $urlparams);
                    $description = get_string('allowsubmissionsshort', 'review');
            ...
            if ($submissionsopen &&
                ...
                $urlparams = array('id' => $this->review->get_course_module()->id,
                                   ...
                                   'page' => $this->currpage);
                $url = new moodle_url('/mod/review/view.php', $urlparams);
                $description = get_string('editsubmission', 'review');
            ...
            if ($USER->id != $row->id &&
                ...
                $urlparams = array('id' => $this->review->get_course_module()->id,
                                   ...
                                   'page' => $this->currpage);
                $url = new moodle_url('/mod/review/view.php', $urlparams);
                $description = get_string('removesubmission', 'review');
        ...
        if (($this->review->get_instance()->duedate ||
                $this->review->get_instance()->cutoffdate) &&
             ...
             $urlparams = array('id' => $this->review->get_course_module()->id,
                                ...
                                'page' => $this->currpage);
             $url = new moodle_url('/mod/review/view.php', $urlparams);
             $description = get_string('grantextension', 'review');
        ...
        if ($row->status == REVIEW_SUBMISSION_STATUS_SUBMITTED &&
                $this->review->get_instance()->submissiondrafts) {
            $urlparams = array('id' => $this->review->get_course_module()->id,
                               'userid' => $row->id,
                               ...
                               'page' => $this->currpage);
            $url = new moodle_url('/mod/review/view.php', $urlparams);
            $description = get_string('reverttodraftshort', 'review');
        ...
        if ($row->status == REVIEW_SUBMISSION_STATUS_DRAFT &&
                $this->review->get_instance()->submissiondrafts &&
                $caneditsubmission &&
                $submissionsopen &&
                $row->id != $USER->id) {
            $urlparams = array('id' => $this->review->get_course_module()->id,
                               ...
                               'page' => $this->currpage);
            $url = new moodle_url('/mod/review/view.php', $urlparams);
            $description = get_string('submitforgrading', 'review');
        ...
        $ismanual = $this->review->get_instance()->attemptreopenmethod == REVIEW_ATTEMPT_REOPEN_METHOD_MANUAL;
        $hassubmission = !empty($row->status);
        $notreopened = $hassubmission && $row->status != REVIEW_SUBMISSION_STATUS_REOPENED;
        $isunlimited = $this->review->get_instance()->maxattempts == REVIEW_UNLIMITED_ATTEMPTS;
        $hasattempts = $isunlimited || $row->attemptnumber < $this->review->get_instance()->maxattempts - 1;
        ...
        if ($ismanual && $hassubmission && $notreopened && $hasattempts) {
            $urlparams = array('id' => $this->review->get_course_module()->id,
                               ...
                               'page' => $this->currpage);
            $url = new moodle_url('/mod/review/view.php', $urlparams);
            $description = get_string('addattempt', 'review');
     ...
     *
     * @param review_plugin $plugin Submission plugin or feedback plugin
     ...
     */
    private function format_plugin_summary_with_link(review_plugin $plugin,
                                                     stdClass $item,
                                                     $returnaction,
                                                     $returnparams) {
        $link = '';
        ...
        $separator = '';
        if ($showviewlink) {
            $viewstr = get_string('view' . substr($plugin->get_subtype(), strlen('review')), 'review');
    ...
    public function other_cols($colname, $row) {
        ...
        if ($plugin->is_visible() && $plugin->is_enabled()) {
            if ($plugin->get_subtype() == 'reviewsubmission') {
                if ($this->review->get_instance()->teamsubmission) {
                    $group = false;
                    $submission = false;
                    ...
                    if ($row->status == REVIEW_SUBMISSION_STATUS_REOPENED) {
                        // For a newly reopened submission - we want to show the previous submission in the table.
                        $submission = $this->review->get_user_submission($row->userid, false, $row->attemptnumber - 1);
                    } else {
                        $submission = new stdClass();
                        $submission->id = $row->submissionid;
                        $submission->timecreated = $row->firstsubmission;
                        $submission->timemodified = $row->timesubmitted;
                        $submission->review = $this->review->get_instance()->id;
                ...
                if ($row->gradeid) {
                    ...
                    $grade->review = $this->review->get_instance()->id;
    ...
    public function get_review_name() {
        return $this->review->get_instance()->name;
    }
    ...
    public function get_course_module_id() {
        return $this->review->get_course_module()->id;
    }
    ...
    public function get_course_id() {
        return $this->review->get_course()->id;
    }
    ...
    public function get_course_context() {
        return $this->review->get_course_context();
    }

    /**
     * Return things to the renderer.
     *
     * @return bool Does this review accept submissions
     */
    public function submissions_enabled() {
        return $this->review->is_any_submission_plugin_enabled();
    }
    ...
    public function can_view_all_grades() {
        $context = $this->review->get_course_context();
    ...
    public function get_sort_columns() {
        ...
        $review = $this->review->get_instance();
        if (empty($review->blindmarking)) {
