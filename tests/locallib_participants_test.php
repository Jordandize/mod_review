...
class mod_review_locallib_participants extends advanced_testcase {
    public function test_list_participants_blind_marking() {
        ...
        review::allocate_unique_ids($review->get_instance()->id);

        $participants = $review->list_participants(null, false);
        ...
        $table = new review_grading_table($review, 10, '', 0, false);
        $table->setup();
        $table->query_db(10);
        ...
        static::helper_add_submission($review, $participants[$keys[1]], $data, 'onlinetext');

        // Review has a private cache. The easiest way to clear this is to create a new instance.
        $review = new review($context, $cm, $course);

        $newparticipants = $review->list_participants(null, false);
        ...
        static::helper_add_submission($review, $participants[$keys[0]], $data, 'onlinetext');
        $review = new review($context, $cm, $course);
        $newparticipants = $review->list_participants(null, false);
    ...
    public function helper_add_submission($review, $user, $data, $type) {
        global $USER;
        ...
        $rc = new ReflectionClass('review');
        $rcm = $rc->getMethod('update_submission');
        $rcm->setAccessible(true);
        $rcm->invokeArgs($review, [$submission, $user->id, true, false]);

        $plugin = $review->get_submission_plugin_by_type($type);
