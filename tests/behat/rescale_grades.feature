...
Feature: Check that the review grade can be rescaled when the max grade is changed
  ...
  Scenario: Update the max grade for an review without rescaling existing grades
  ...
  Scenario: Update an review without touching the max grades
    Given I navigate to "Edit settings" in current page administration
    And I expand all fieldsets
    And I set the field "Rescale existing grades" to "No"
    And I set the field "Maximum grade" to "80"
  ...
  Scenario: Update the max grade for an review rescaling existing grades
    Given I navigate to "Edit settings" in current page administration
    And I expand all fieldsets
    And I set the field "Rescale existing grades" to "Yes"
  ...
  Scenario: Rescaling should not produce negative grades
    ...
    And I follow "Review: Test review name"
    And I navigate to "Edit settings" in current page administration
    And I expand all fieldsets
    And I set the field "Rescale existing grades" to "Yes"
    And I set the field "Maximum grade" to "50"
