@mod @mod_review
Feature: View the grading status of an review
  In order to test the grading status for reviews is displaying correctly
  As a student
  ...
  @javascript
  Scenario: View the grading status for an review with marking workflow enabled
    # Add the review.
    ...
    # The grade should also remain displayed as it's stored in the review DB tables, but the final grade should be empty.
  ...
  @javascript
  Scenario: View the grading status for an review with marking workflow disabled
    # Add the review.
    And I log in as "teacher1"
    And I am on "Course 1" course homepage with editing mode on
    And I add a "Review" to section "1" and I fill the form with:
