...
/**
 * Unit tests for (some of) mod/review/markerallocaion_test.php.
 *
 * @package    mod_review
 * @category   test
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_review_markerallocation_testcase extends advanced_testcase {
    ...
    public function test_markerusers() {
        ...
        $reviewelement = $this->getDataGenerator()->create_module('review', $record, $modulesettings);

        $coursesectionid = course_add_cm_to_section($this->course->id, $reviewelement->id, 1);
        ...
        if (!empty($managerrole)) {
            // By default the context of the system is reviewed.
            $idreview = $this->getDataGenerator()->role_review($managerrole->id, $user4->id);
        }
        ...
        $oldmarkers = get_users_by_capability(context_course::instance($this->course->id), 'mod/review:grade', '', $sort);
        // New code, it must return 2 users: teacher1 and teacher2.
        $newmarkers = get_enrolled_users(context_course::instance($this->course->id), 'mod/review:grade', 0, 'u.*', $sort);
